...
| `principal_holding_filing_trace` | real-derived 13F filing-trace C1 task | oracle score regret 0 on one Berkshire 2025Q3->2026Q1 public SEC-derived trace; percent-change baseline regret 0.7555; market-value, low-turnover, max-position, and previous-trend baselines all regret 1; oracle margin 0.7555; first live `nano`/`mini`/`gpt-5.5` repeat sweep is stable-oracle for all aliases | first C1 trace populated from actual SEC 13F rows; depth value is not case count, but the test-derived question. This initial real trace validates the harness but is clean enough for all aliases, and PR 114 shows that the useful next ambiguity is relative-share salience versus dollar-material action rather than more synthetic variants |
| `principal_holding_filing_trace_raw` | raw-row real-derived 13F filing-trace C1 task | oracle score regret 0 on the same Berkshire public SEC-derived trace; percent-change baseline regret 0.7555; market-value, low-turnover, max-position, and previous-trend baselines all regret 1; oracle margin 0.7555; final live `nano`/`mini`/`gpt-5.5` repeat sweep is stable-oracle after explicit dollar-material wording | removes the precomputed candidate-change scaffold from PR 113 and requires inferring the dollar-material share-action issuer from period filing rows. The first raw live probes surfaced a real failure hypothesis: smaller aliases can read the largest percentage reduction (`sec_constellation`) as the salient action even when the economic target is the largest approximate dollar value of share-count change (`sec_chevron`) |
...
| Real-derived C1 filing-trace oracle smoke | `python -m aeread_lab.cli --task principal_holding_filing_trace --agent offline:oracle --no-cache` | n=1, score regret 0, accuracy 1.00, oracle margin 0.7555, and all named shortcut miss rates, including percent-change miss, are 0 for the oracle |
| Real-derived C1 filing-trace shortcut sweep | `python -m aeread_lab.cli --sweep --task principal_holding_filing_trace --agents offline:oracle,offline:market_value,offline:low_turnover,offline:max_position,offline:trend,offline:percent_change --no-cache` | oracle rank 1 with score regret 0; percent-change shortcut regret 0.7555; market-value, low-turnover, max-position, and previous-trend shortcut baselines all score regret 1; this is a depth probe for dollar-material share action versus filing-value drift and percentage-change salience, not a new breadth target |
...
| Raw-row real-derived C1 filing-trace tests | `python -m pytest tests/test_tasks.py -k 'filing_trace or principal_holding'` | 13 selected tests passed; raw prompt includes final-period `filing_row` rows and omits `candidate_trade` / `prior_shares` scaffold fields |
| Raw-row real-derived C1 filing-trace shortcut sweep | `python -m aeread_lab.cli --sweep --task principal_holding_filing_trace_raw --agents offline:oracle,offline:market_value,offline:low_turnover,offline:max_position,offline:trend,offline:percent_change --no-cache` | oracle rank 1 with score regret 0; percent-change shortcut regret 0.7555 with modal `sec_constellation`; market-value, low-turnover, max-position, and previous-trend shortcut baselines all score regret 1 |
