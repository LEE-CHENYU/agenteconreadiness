...
| `pricing_counterfactual` | pricing posterior evidence perturbations | oracle posterior price error 0.00125 across 4 prompts; stale-price baseline price error 4.53125 with counterfactual-shift miss 1.00 and sticky-base-price 1.00; rounded baseline price error 2.03125 | implements the proposal's pricing sales-evidence perturbation variant: changed evidence should change the posterior revenue-maximizing price |
...
| Sampled all-task oracle smoke | `python3 -m aeread_lab.cli --task all --agent offline:oracle --limit 1 --no-cache` | all 27 task runners execute with first deterministic case/gamble only; scam remains instrumented |
