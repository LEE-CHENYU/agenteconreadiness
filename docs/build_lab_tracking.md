...
- C1 filing inference: implicit corporate-action adjustment is the active
  ...
  `nano` and `gpt-5.5`. PR 122 restores a separate corporate-action registry
  while keeping the conflicting-value rows; this stabilizes `gpt-5.5` but leaves
  one `nano` artifact-blind miss in repeat-6. PR 123 adds stale, unconfirmed,
  non-split, and unmatched registry distractors while keeping the correct
  confirmed target-period split records, and repeat-6 stabilizes `nano` and
  ...
  repeat-3 and `gpt-5.5` solves repeat-6. PR 126 removes the explicit
  conflict-warning cue while keeping the wrong confirmed record; now `nano`
  and `mini` are metadata-trusting-modal while `gpt-5.5` remains stable-oracle.
  PR 127 adds repeated issuer history without restoring the warning, and the
  smaller aliases remain metadata-trusting-modal. PR 128 adds a generic
  row-ratio validation process without restoring the explicit conflict warning;
  all aliases become stable-oracle in repeat-6. The current question is
  therefore depth within registry grounding: whether source reliability can
  induce metadata-row cross-checking without a procedural validation scaffold.
  PR 129 tests direct-vs-backfill source provenance without the procedure;
  `nano` and `mini` still choose metadata-trusting actions in repeat-6, while
  `gpt-5.5` remains stable-oracle. The next C1 question is real
  corporate-action context or a more natural source-grounding surface, not more
  synthetic provenance labels. PR 130 adds natural source packets around the
  same confirmed false row; this makes all aliases metadata-trusting-modal or
  worse in repeat-6. The current question is now whether the `confirmed`
  registry status itself is the crutch, or whether real corporate-action source
  rows can induce validation without a printed procedure. PR 131 tests the
  status-crutch branch by marking the same false third-party row `unverified`;
  ...
  13F inversion noise where source quality is not synthetically clean. PR 135
  runs that component ablation on the same case. Primary-only gives a partial
  recovery: `mini` and `gpt-5.5` are stable-oracle in repeat-6, while `nano` is
  oracle-modal with one metadata-trusting/status-blind miss. Backfill-only is
  weaker: `mini` becomes metadata-trusting/status-blind-modal and `nano` is
  unstable with metadata misses. Ratio-only rescues `mini` and `gpt-5.5`, but
  leaves `nano` metadata-trusting/status-blind-modal. The full audit remains the
  only tested source-audit surface that makes all aliases stable-oracle, so the
  next uncovered direction is a real-source full-audit probe rather than more
  single-component synthetic variants.
...
| 126 | `review/126-unmarked-conflicting-metadata` | unmarked wrong-confirmed registry control | removes the explicit conflict warning from PR125 while keeping the false confirmed split; `nano` and `mini` become metadata-trusting-modal, while `gpt-5.5` remains stable-oracle |
| 127 | `review/127-history-conflicting-metadata` | repeated-history wrong-confirmed registry control | adds an earlier filing period for every issuer while keeping the false confirmed split unmarked; `nano` and `mini` remain metadata-trusting-modal, while `gpt-5.5` remains stable-oracle |
| 128 | `review/128-metadata-validation-process` | generic metadata-validation process scaffold | adds row-ratio validation process without restoring the conflict warning; repeat-6 makes `nano`, `mini`, and `gpt-5.5` stable-oracle |
| 129 | `review/129-metadata-source-provenance` | source-provenance registry scaffold | replaces the validation process with direct-vs-backfill source labels; repeat-6 keeps `nano` stable metadata-trusting and `mini` metadata-trusting-modal while `gpt-5.5` remains stable-oracle |
| 130 | `review/130-metadata-source-context` | natural source-packet registry scaffold | replaces source labels with natural issuer/exchange and third-party source packets; repeat-6 makes `nano` stable metadata-trusting and `mini`/`gpt-5.5` metadata-trusting-modal |
...
| 135 | `review/135-source-audit-component-ablation` | source-audit component ablations | splits PR134's audit into primary-only, backfill-only, and ratio-only cues on the same case; full audit is the only tested variant that makes all aliases stable-oracle |
...
| `principal_holding_filing_artifact_metadata_unmarked_conflict` | conflicting-value artifact C1 control with an unmarked wrong confirmed stock-split registry record | oracle score regret 0; second-best regret 0.0645161; percent-change regret 0.774194; artifact-blind, market-value, and metadata-trusting baselines regret 1; oracle margin 0.0645161 | removes PR125's explicit registry-conflict warning while keeping the same false confirmed split. Live repeat-3 has `nano` stable metadata-trusting non-oracle, `mini` unstable-non-oracle-modal, and `gpt-5.5` stable-oracle; repeat-6 keeps `gpt-5.5` stable while `nano` and `mini` are metadata-trusting-modal |
| `principal_holding_filing_artifact_metadata_history_conflict` | repeated-history artifact C1 control with an unmarked wrong confirmed stock-split registry record | oracle score regret 0; second-best regret 0.0645161; percent-change regret 0.774194; artifact-blind, market-value, and metadata-trusting baselines regret 1; oracle margin 0.0645161 | adds a prior issuer-level filing period to PR126 without adding a conflict warning. Live repeat-3 and repeat-6 keep `gpt-5.5` stable-oracle while `nano` and `mini` remain metadata-trusting-modal, so synthetic history alone does not fix the smaller-alias validation failure |
| `principal_holding_filing_artifact_metadata_validation_process` | repeated-history artifact C1 control with a row-ratio metadata-validation process | oracle score regret 0; metadata-validator regret 0; second-best regret 0.0645161; percent-change regret 0.774194; artifact-blind, market-value, and metadata-trusting baselines regret 1; oracle margin 0.0645161 | adds a generic process cue to compute observed row ratios before accepting registry splits, but does not restore the explicit conflict warning. Live repeat-3 has `nano` and `gpt-5.5` stable-oracle while `mini` has one percent-change miss; repeat-6 makes all three aliases stable-oracle, so the PR126/127 smaller-alias failure was process-cue sensitivity rather than missing row evidence |
| `principal_holding_filing_artifact_metadata_source_provenance` | repeated-history artifact C1 control with source-labeled stock-split registry rows | oracle score regret 0; source-reliability regret 0; second-best regret 0.0645161; percent-change regret 0.774194; artifact-blind, market-value, and metadata-trusting baselines regret 1; oracle margin 0.0645161 | replaces PR128's row-ratio validation process with source provenance labels that distinguish direct issuer/exchange notices from lower-reliability third-party backfill metadata. Live repeat-3 has `gpt-5.5` stable-oracle, `mini` stable metadata-trusting, and `nano` metadata-trusting-modal; repeat-6 keeps `gpt-5.5` stable-oracle while `nano` is stable metadata-trusting and `mini` is metadata-trusting-modal. Source labels alone do not fix the smaller-alias validation failure |
| `principal_holding_filing_artifact_metadata_source_context` | repeated-history artifact C1 control with natural corporate-action source packets | oracle score regret 0; source-reliability regret 0; second-best regret 0.0645161; percent-change regret 0.774194; artifact-blind, market-value, and metadata-trusting baselines regret 1; oracle margin 0.0645161 | replaces PR129's terse source labels with natural source packets: direct issuer/exchange notices for true splits and third-party backfill text without issuer/exchange attachments for the false split. Live repeat-3 makes all aliases stable metadata-trusting; repeat-6 keeps `nano` stable metadata-trusting while `mini` and `gpt-5.5` are metadata-trusting-modal. Richer source context increases registry deference rather than replacing the validation process |
...
| `principal_holding_filing_artifact_metadata_source_audit_primary_only` | source-audit ablation with only primary-source recognition | oracle score regret 0; source-reliability and metadata-source-trusting regret 0; second-best regret 0.0645161; percent-change regret 0.774194; metadata-trusting, metadata-naive, artifact-blind, and market-value baselines regret 1; oracle margin 0.0645161 | isolates issuer/exchange/transfer-agent attachment recognition from backfill skepticism and row-ratio reconciliation. Repeat-6 makes `mini` and `gpt-5.5` stable-oracle, while `nano` is oracle-modal with one attributed metadata-trusting/status-blind miss |
| `principal_holding_filing_artifact_metadata_source_audit_backfill_only` | source-audit ablation with only third-party backfill skepticism | oracle score regret 0; source-reliability and metadata-source-trusting regret 0; second-best regret 0.0645161; percent-change regret 0.774194; metadata-trusting, metadata-naive, artifact-blind, and market-value baselines regret 1; oracle margin 0.0645161 | isolates skepticism toward unattached third-party backfill rows. Repeat-6 keeps only `gpt-5.5` stable-oracle; `nano` is unstable-oracle-modal with metadata-trusting/status-blind misses, and `mini` is metadata-trusting/status-blind-modal |
| `principal_holding_filing_artifact_metadata_source_audit_ratio_only` | source-audit ablation with only row-ratio reconciliation | oracle score regret 0; source-reliability and metadata-source-trusting regret 0; second-best regret 0.0645161; percent-change regret 0.774194; metadata-trusting, metadata-naive, artifact-blind, and market-value baselines regret 1; oracle margin 0.0645161 | isolates a light instruction to compare claimed split ratios with filing-row share-count ratios. Repeat-6 makes `mini` and `gpt-5.5` stable-oracle, while `nano` is metadata-trusting/status-blind-modal. The full PR134 audit is still needed for the smallest alias |
...
| PR122 focused metadata artifact tests | `python -m pytest tests/test_tasks.py -k 'filing_artifact or filing_trace or principal_holding'` | 40 selected tests passed; the prompt uses a separate `corporate_action` registry instead of artifact notes, and the offline oracle parses registry ratios |
| PR122 metadata shortcut sweep | `python -m aeread_lab.cli --sweep --task principal_holding_filing_artifact_metadata --agents offline:oracle,offline:artifact_blind,offline:market_value,offline:percent_change,offline:second_best --no-cache` | oracle rank 1; second-best regret 0.0645161; percent-change regret 0.774194; artifact-blind and market-value regret 1 |
| PR122 metadata live repeat | `AEREAD_OPENAI_MAX_OUTPUT_TOKENS=8192 python -m aeread_lab.cli --sweep --task principal_holding_filing_artifact_metadata --agents openai:nano,openai:mini,openai:gpt-5.5 --repeat 3 --case multi_artifact_close_runner_up --no-cache` | all three aliases are stable-oracle with parse floor 1.00, score regret 0, accuracy 1.00, and modal `sec_stress_b` |
| PR122 metadata live follow-up | `AEREAD_OPENAI_MAX_OUTPUT_TOKENS=8192 python -m aeread_lab.cli --sweep --task principal_holding_filing_artifact_metadata --agents openai:nano,openai:gpt-5.5 --repeat 6 --case multi_artifact_close_runner_up --no-cache` | `gpt-5.5` is stable-oracle; `nano` is unstable-oracle-modal with mean score regret 0.166667, oracle-hit 0.833333, and one `artifact_blind` choice |
| PR122 full pytest | `python -m pytest` | 287 tests passed |
| PR122 full all-task oracle | `python -m aeread_lab.cli --task all --agent offline:oracle --no-cache` | all 92 current task runners execute; oracle path remains clean, including `principal_holding_filing_artifact_metadata` with `n=1`, score regret 0, accuracy 1.00, and oracle margin 0.0645 |
| PR123 focused noisy-metadata tests | `python -m pytest tests/test_tasks.py -k 'metadata_noisy or filing_artifact_metadata_noisy or filing_artifact_metadata'` | 8 selected tests passed; the prompt includes stale/unconfirmed/unmatched registry distractors, and the naive metadata baseline chooses the unconfirmed registry distractor |
| PR123 noisy-metadata shortcut sweep | `python -m aeread_lab.cli --sweep --task principal_holding_filing_artifact_metadata_noisy --agents offline:oracle,offline:artifact_blind,offline:market_value,offline:percent_change,offline:second_best,offline:metadata_naive --no-cache` | oracle rank 1; second-best and metadata-naive regret 0.0645161; percent-change regret 0.774194; artifact-blind and market-value regret 1 |
| PR123 noisy-metadata live repeat | `AEREAD_OPENAI_MAX_OUTPUT_TOKENS=8192 python -m aeread_lab.cli --sweep --task principal_holding_filing_artifact_metadata_noisy --agents openai:nano,openai:mini,openai:gpt-5.5 --repeat 3 --case multi_artifact_noisy_registry_close_runner_up --no-cache` | all three aliases are stable-oracle with parse floor 1.00, score regret 0, accuracy 1.00, and modal `sec_stress_b` |
| PR123 noisy-metadata live follow-up | `AEREAD_OPENAI_MAX_OUTPUT_TOKENS=8192 python -m aeread_lab.cli --sweep --task principal_holding_filing_artifact_metadata_noisy --agents openai:nano,openai:gpt-5.5 --repeat 6 --case multi_artifact_noisy_registry_close_runner_up --no-cache` | `nano` and `gpt-5.5` are stable-oracle with parse floor 1.00, score regret 0, accuracy 1.00, and modal `sec_stress_b` |
| PR123 full pytest | `python -m pytest` | 291 tests passed |
| PR123 full all-task oracle | `python -m aeread_lab.cli --task all --agent offline:oracle --no-cache` | all 93 current task runners execute; oracle path remains clean, including `principal_holding_filing_artifact_metadata_noisy` with `n=1`, score regret 0, accuracy 1.00, and oracle margin 0.0645 |
| PR124 focused partial-metadata tests | `python -m pytest tests/test_tasks.py -k 'metadata_partial or filing_artifact_metadata_partial or metadata_noisy or filing_artifact_metadata'` | 12 selected tests passed; the prompt marks incomplete registry coverage and the metadata-only baseline chooses the omitted split artifact |
| PR124 partial-metadata shortcut sweep | `python -m aeread_lab.cli --sweep --task principal_holding_filing_artifact_metadata_partial --agents offline:oracle,offline:artifact_blind,offline:market_value,offline:percent_change,offline:second_best,offline:metadata_only --no-cache` | oracle rank 1; second-best regret 0.0645161; percent-change regret 0.774194; artifact-blind, market-value, and metadata-only regret 1 |
| PR124 partial-metadata live repeat | `AEREAD_OPENAI_MAX_OUTPUT_TOKENS=8192 python -m aeread_lab.cli --sweep --task principal_holding_filing_artifact_metadata_partial --agents openai:nano,openai:mini,openai:gpt-5.5 --repeat 3 --case multi_artifact_partial_registry_close_runner_up --no-cache` | `mini` and `gpt-5.5` are stable-oracle; `nano` is unstable-non-oracle-modal with mean score regret 0.666667, accuracy 0.333333, and artifact-blind modal choice |
...
| PR126 focused unmarked-conflict tests | `python -m pytest tests/test_tasks.py -k 'metadata_unmarked_conflict or unmarked_conflict or metadata_conflict or filing_artifact_metadata_conflict or filing_artifact_metadata'` | 20 selected tests passed; the prompt keeps the false confirmed split but removes the explicit conflict warning |
| PR126 broader C1 filing tests | `python -m pytest tests/test_tasks.py -k 'filing_artifact or filing_trace or principal_holding'` | 56 selected tests passed |
| PR126 unmarked-conflict shortcut sweep | `python -m aeread_lab.cli --sweep --task principal_holding_filing_artifact_metadata_unmarked_conflict --agents offline:oracle,offline:artifact_blind,offline:market_value,offline:percent_change,offline:second_best,offline:metadata_trusting --no-cache` | oracle rank 1; second-best regret 0.0645161; percent-change regret 0.774194; artifact-blind, market-value, and metadata-trusting regret 1 |
| PR126 unmarked-conflict live repeat | `AEREAD_OPENAI_MAX_OUTPUT_TOKENS=8192 python -m aeread_lab.cli --sweep --task principal_holding_filing_artifact_metadata_unmarked_conflict --agents openai:nano,openai:mini,openai:gpt-5.5 --repeat 3 --case multi_artifact_unmarked_conflicting_registry_close_runner_up --no-cache` | `gpt-5.5` is stable-oracle; `nano` is stable-non-oracle with metadata-trusting modal choice and mean regret 1; `mini` is unstable-non-oracle-modal with metadata-trusting modal choice and mean regret 0.924731 |
| PR126 unmarked-conflict live follow-up | `AEREAD_OPENAI_MAX_OUTPUT_TOKENS=8192 python -m aeread_lab.cli --sweep --task principal_holding_filing_artifact_metadata_unmarked_conflict --agents openai:nano,openai:mini,openai:gpt-5.5 --repeat 6 --case multi_artifact_unmarked_conflicting_registry_close_runner_up --no-cache` | `gpt-5.5` is stable-oracle; `nano` is unstable-non-oracle-modal with mean regret 0.833333 and choice refs `metadata_trusting:5`; `mini` is unstable-non-oracle-modal with mean regret 0.666667 and choice refs `metadata_trusting:4` |
| PR126 full all-task oracle | `python -m aeread_lab.cli --task all --agent offline:oracle --no-cache` | all 96 current task runners execute; oracle path remains clean, including `principal_holding_filing_artifact_metadata_unmarked_conflict` with `n=1`, score regret 0, accuracy 1.00, and oracle margin 0.0645 |
| PR126 full pytest | `python -m pytest` | 303 tests passed |
| PR127 focused history-conflict tests | `python -m pytest tests/test_tasks.py -k 'metadata_history_conflict or history_conflict or metadata_unmarked_conflict or filing_artifact_metadata'` | 24 selected tests passed; the prompt adds repeated issuer history, keeps the false confirmed split, and does not restore the explicit conflict warning |
| PR127 broader C1 filing tests | `python -m pytest tests/test_tasks.py -k 'filing_artifact or filing_trace or principal_holding'` | 60 selected tests passed |
| PR127 history-conflict shortcut sweep | `python -m aeread_lab.cli --sweep --task principal_holding_filing_artifact_metadata_history_conflict --agents offline:oracle,offline:artifact_blind,offline:market_value,offline:percent_change,offline:second_best,offline:metadata_trusting --no-cache` | oracle rank 1; second-best regret 0.0645161; percent-change regret 0.774194; artifact-blind, market-value, and metadata-trusting regret 1 |
| PR127 history-conflict live repeat | `AEREAD_OPENAI_MAX_OUTPUT_TOKENS=8192 python -m aeread_lab.cli --sweep --task principal_holding_filing_artifact_metadata_history_conflict --agents openai:nano,openai:mini,openai:gpt-5.5 --repeat 3 --case multi_artifact_history_conflicting_registry_close_runner_up --no-cache` | `gpt-5.5` is stable-oracle; `nano` is stable metadata-trusting non-oracle with mean regret 1; `mini` is unstable-non-oracle-modal with mean regret 0.666667 and choice refs `metadata_trusting:2` |
| PR127 history-conflict live follow-up | `AEREAD_OPENAI_MAX_OUTPUT_TOKENS=8192 python -m aeread_lab.cli --sweep --task principal_holding_filing_artifact_metadata_history_conflict --agents openai:nano,openai:mini,openai:gpt-5.5 --repeat 6 --case multi_artifact_history_conflicting_registry_close_runner_up --no-cache` | `gpt-5.5` is stable-oracle; `nano` and `mini` are unstable-non-oracle-modal with mean regret 0.833333 and choice refs `metadata_trusting:5` each |
| PR127 full all-task oracle | `python -m aeread_lab.cli --task all --agent offline:oracle --no-cache` | all 97 current task runners execute; oracle path remains clean, including `principal_holding_filing_artifact_metadata_history_conflict` with `n=1`, score regret 0, accuracy 1.00, and oracle margin 0.0645 |
| PR127 full pytest | `python -m pytest` | 307 tests passed |
| PR128 focused validation-process tests | `python -m pytest tests/test_tasks.py -k 'validation_process or metadata_history_conflict or history_conflict or metadata_unmarked_conflict or filing_artifact_metadata'` | 28 selected tests passed; the prompt adds a row-ratio validation process, keeps the false confirmed split, and does not restore the explicit conflict warning |
| PR128 broader C1 filing tests | `python -m pytest tests/test_tasks.py -k 'filing_artifact or filing_trace or principal_holding'` | 64 selected tests passed |
| PR128 validation-process shortcut sweep | `python -m aeread_lab.cli --sweep --task principal_holding_filing_artifact_metadata_validation_process --agents offline:oracle,offline:metadata_validator,offline:artifact_blind,offline:market_value,offline:percent_change,offline:second_best,offline:metadata_trusting --no-cache` | oracle and metadata-validator rank first with score regret 0; second-best regret 0.0645161; percent-change regret 0.774194; artifact-blind, market-value, and metadata-trusting regret 1 |
| PR128 validation-process live repeat | `AEREAD_OPENAI_MAX_OUTPUT_TOKENS=8192 python -m aeread_lab.cli --sweep --task principal_holding_filing_artifact_metadata_validation_process --agents openai:nano,openai:mini,openai:gpt-5.5 --repeat 3 --case multi_artifact_validation_process_conflicting_registry_close_runner_up --no-cache` | `nano` and `gpt-5.5` are stable-oracle; `mini` is unstable-oracle-modal with mean regret 0.258065, accuracy 0.666667, and one attributed `percent_change` choice |
| PR128 validation-process live follow-up | `AEREAD_OPENAI_MAX_OUTPUT_TOKENS=8192 python -m aeread_lab.cli --sweep --task principal_holding_filing_artifact_metadata_validation_process --agents openai:nano,openai:mini,openai:gpt-5.5 --repeat 6 --case multi_artifact_validation_process_conflicting_registry_close_runner_up --no-cache` | `nano`, `mini`, and `gpt-5.5` are stable-oracle with parse floor 1.00, score regret 0, accuracy 1.00, and modal `sec_stress_b` |
| PR128 full all-task oracle | `python -m aeread_lab.cli --task all --agent offline:oracle --no-cache` | all 98 current task runners execute; oracle path remains clean, including `principal_holding_filing_artifact_metadata_validation_process` with `n=1`, score regret 0, accuracy 1.00, and oracle margin 0.0645 |
| PR128 full pytest | `python -m pytest` | 311 tests passed |
| PR129 focused source-provenance tests | `python -m pytest tests/test_tasks.py -k 'source_provenance or validation_process or metadata_history_conflict or history_conflict or metadata_unmarked_conflict or filing_artifact_metadata'` | 32 selected tests passed; the prompt adds source-provenance labels, keeps the false confirmed split, and omits both the explicit conflict warning and the row-ratio validation process |
| PR129 broader C1 filing tests | `python -m pytest tests/test_tasks.py -k 'filing_artifact or filing_trace or principal_holding'` | 68 selected tests passed |
| PR129 source-provenance shortcut sweep | `python -m aeread_lab.cli --sweep --task principal_holding_filing_artifact_metadata_source_provenance --agents offline:oracle,offline:source_reliability,offline:artifact_blind,offline:market_value,offline:percent_change,offline:second_best,offline:metadata_trusting --no-cache` | oracle and source-reliability rank first with score regret 0; second-best regret 0.0645161; percent-change regret 0.774194; artifact-blind, market-value, and metadata-trusting regret 1 |
| PR129 source-provenance live repeat | `AEREAD_OPENAI_MAX_OUTPUT_TOKENS=8192 python -m aeread_lab.cli --sweep --task principal_holding_filing_artifact_metadata_source_provenance --agents openai:nano,openai:mini,openai:gpt-5.5 --repeat 3 --case multi_artifact_source_provenance_conflicting_registry_close_runner_up --no-cache` | `gpt-5.5` is stable-oracle; `mini` is stable metadata-trusting with mean regret 1; `nano` is unstable-non-oracle-modal with mean regret 0.666667 and choice refs `metadata_trusting:2` |
| PR129 source-provenance live follow-up | `AEREAD_OPENAI_MAX_OUTPUT_TOKENS=8192 python -m aeread_lab.cli --sweep --task principal_holding_filing_artifact_metadata_source_provenance --agents openai:nano,openai:mini,openai:gpt-5.5 --repeat 6 --case multi_artifact_source_provenance_conflicting_registry_close_runner_up --no-cache` | `gpt-5.5` is stable-oracle; `nano` is stable metadata-trusting with mean regret 1 and choice refs `metadata_trusting:6`; `mini` is unstable-non-oracle-modal with mean regret 0.833333 and choice refs `metadata_trusting:5` |
| PR129 full all-task oracle | `python -m aeread_lab.cli --task all --agent offline:oracle --no-cache` | all 99 current task runners execute; oracle path remains clean, including `principal_holding_filing_artifact_metadata_source_provenance` with `n=1`, score regret 0, accuracy 1.00, and oracle margin 0.0645 |
| PR129 full pytest | `python -m pytest` | 315 tests passed |
| PR130 focused source-context tests | `python -m pytest tests/test_tasks.py -k 'source_context or source_provenance or validation_process or metadata_history_conflict or history_conflict or metadata_unmarked_conflict or filing_artifact_metadata'` | 36 selected tests passed; the prompt adds natural source packets, keeps the false confirmed split, and omits both the explicit conflict warning and the row-ratio validation process |
| PR130 broader C1 filing tests | `python -m pytest tests/test_tasks.py -k 'filing_artifact or filing_trace or principal_holding'` | 72 selected tests passed |
| PR130 source-context shortcut sweep | `python -m aeread_lab.cli --sweep --task principal_holding_filing_artifact_metadata_source_context --agents offline:oracle,offline:source_reliability,offline:artifact_blind,offline:market_value,offline:percent_change,offline:second_best,offline:metadata_trusting --no-cache` | oracle and source-reliability rank first with score regret 0; second-best regret 0.0645161; percent-change regret 0.774194; artifact-blind, market-value, and metadata-trusting regret 1 |
| PR130 source-context live repeat | `AEREAD_OPENAI_MAX_OUTPUT_TOKENS=8192 python -m aeread_lab.cli --sweep --task principal_holding_filing_artifact_metadata_source_context --agents openai:nano,openai:mini,openai:gpt-5.5 --repeat 3 --case multi_artifact_source_context_conflicting_registry_close_runner_up --no-cache` | all three aliases are stable metadata-trusting non-oracle with mean regret 1 and choice refs `metadata_trusting:3` each |
| PR130 source-context live follow-up | `AEREAD_OPENAI_MAX_OUTPUT_TOKENS=8192 python -m aeread_lab.cli --sweep --task principal_holding_filing_artifact_metadata_source_context --agents openai:nano,openai:mini,openai:gpt-5.5 --repeat 6 --case multi_artifact_source_context_conflicting_registry_close_runner_up --no-cache` | `nano` is stable metadata-trusting with mean regret 1 and choice refs `metadata_trusting:6`; `mini` is unstable-non-oracle-modal with mean regret 0.833333 and choice refs `metadata_trusting:5`; `gpt-5.5` is unstable-non-oracle-modal with mean regret 0.666667 and choice refs `metadata_trusting:4` |
| PR130 full all-task oracle | `python -m aeread_lab.cli --task all --agent offline:oracle --no-cache` | all 100 current task runners execute; oracle path remains clean, including `principal_holding_filing_artifact_metadata_source_context` with `n=1`, score regret 0, accuracy 1.00, and oracle margin 0.0645 |
| PR130 full pytest | `python -m pytest` | 319 tests passed |
| PR131 focused source-status-ablation tests | `python -m pytest tests/test_tasks.py -k 'source_status_ablation or source_context or source_provenance or validation_process or metadata_history_conflict or history_conflict or metadata_unmarked_conflict or filing_artifact_metadata'` | 40 selected tests passed; the prompt keeps natural source packets but changes the false third-party row from `status=confirmed` to `status=unverified` |
| PR131 broader C1 filing tests | `python -m pytest tests/test_tasks.py -k 'filing_artifact or filing_trace or principal_holding'` | 76 selected tests passed |
| PR131 source-status-ablation shortcut sweep | `python -m aeread_lab.cli --sweep --task principal_holding_filing_artifact_metadata_source_status_ablation --agents offline:oracle,offline:source_reliability,offline:metadata_trusting,offline:metadata_naive,offline:artifact_blind,offline:market_value,offline:percent_change,offline:second_best --no-cache` | oracle, source-reliability, and metadata-trusting rank first with score regret 0; second-best regret 0.0645161; percent-change regret 0.774194; metadata-naive, artifact-blind, and market-value regret 1 |
| PR131 source-status-ablation live repeat | `AEREAD_OPENAI_MAX_OUTPUT_TOKENS=8192 python -m aeread_lab.cli --sweep --task principal_holding_filing_artifact_metadata_source_status_ablation --agents openai:nano,openai:mini,openai:gpt-5.5 --repeat 3 --case multi_artifact_source_status_ablation_conflicting_registry_close_runner_up --no-cache` | `nano`, `mini`, and `gpt-5.5` are stable-oracle with parse floor 1.00, score regret 0, accuracy 1.00, and modal `sec_stress_b` |
| PR131 source-status-ablation live follow-up | `AEREAD_OPENAI_MAX_OUTPUT_TOKENS=8192 python -m aeread_lab.cli --sweep --task principal_holding_filing_artifact_metadata_source_status_ablation --agents openai:nano,openai:mini,openai:gpt-5.5 --repeat 6 --case multi_artifact_source_status_ablation_conflicting_registry_close_runner_up --no-cache` | `nano`, `mini`, and `gpt-5.5` remain stable-oracle with parse floor 1.00, score regret 0, accuracy 1.00, and modal `sec_stress_b` |
| PR131 full all-task oracle | `python -m aeread_lab.cli --task all --agent offline:oracle --no-cache` | all 101 current task runners execute; oracle path remains clean, including `principal_holding_filing_artifact_metadata_source_status_ablation` with `n=1`, score regret 0, accuracy 1.00, and oracle margin 0.0645 |
| PR131 full pytest | `python -m pytest` | 323 tests passed |
| PR132 focused source-status-neutral tests | `python -m pytest tests/test_tasks.py -k 'source_status_neutral or source_status_ablation or source_context or source_provenance or validation_process or metadata_history_conflict or history_conflict or metadata_unmarked_conflict or filing_artifact_metadata'` | 44 selected tests passed; the prompt keeps natural source packets but removes all registry `status=` labels |
| PR132 broader C1 filing tests | `python -m pytest tests/test_tasks.py -k 'filing_artifact or filing_trace or principal_holding'` | 80 selected tests passed |
| PR132 source-status-neutral shortcut sweep | `python -m aeread_lab.cli --sweep --task principal_holding_filing_artifact_metadata_source_status_neutral --agents offline:oracle,offline:source_reliability,offline:metadata_source_trusting,offline:metadata_trusting,offline:metadata_naive,offline:artifact_blind,offline:market_value,offline:percent_change,offline:second_best --no-cache` | oracle, source-reliability, and metadata-source-trusting rank first with score regret 0; second-best regret 0.0645161; percent-change regret 0.774194; metadata-trusting, metadata-naive, artifact-blind, and market-value regret 1 |
| PR132 source-status-neutral live repeat | `AEREAD_OPENAI_MAX_OUTPUT_TOKENS=8192 python -m aeread_lab.cli --sweep --task principal_holding_filing_artifact_metadata_source_status_neutral --agents openai:nano,openai:mini,openai:gpt-5.5 --repeat 3 --case multi_artifact_source_status_neutral_conflicting_registry_close_runner_up --no-cache` | `gpt-5.5` is stable-oracle; `mini` is unstable-oracle-modal with mean regret 0.258065 and one percent-change miss; `nano` is stable metadata-trusting/status-blind with mean regret 1 |
| PR132 source-status-neutral live follow-up | `AEREAD_OPENAI_MAX_OUTPUT_TOKENS=8192 python -m aeread_lab.cli --sweep --task principal_holding_filing_artifact_metadata_source_status_neutral --agents openai:nano,openai:mini,openai:gpt-5.5 --repeat 6 --case multi_artifact_source_status_neutral_conflicting_registry_close_runner_up --no-cache` | `gpt-5.5` remains stable-oracle; `mini` is metadata-trusting/status-blind-modal with mean regret 0.666667; `nano` is metadata-trusting/status-blind-modal with mean regret 0.833333 |
| PR132 full all-task oracle | `python -m aeread_lab.cli --task all --agent offline:oracle --no-cache` | all 102 current task runners execute; oracle path remains clean, including `principal_holding_filing_artifact_metadata_source_status_neutral` with `n=1`, score regret 0, accuracy 1.00, and oracle margin 0.0645 |
| PR132 full pytest | `python -m pytest` | 327 tests passed |
| PR133 focused source-status-neutral-intro tests | `python -m pytest tests/test_tasks.py -k 'source_status_neutral_intro or source_status_neutral or source_status_ablation or source_context or source_provenance or validation_process or metadata_history_conflict or history_conflict or metadata_unmarked_conflict or filing_artifact_metadata'` | 48 selected tests passed; the prompt keeps status-neutral source packets and removes generic `confirmed` wording |
| PR133 broader C1 filing tests | `python -m pytest tests/test_tasks.py -k 'filing_artifact or filing_trace or principal_holding'` | 84 selected tests passed |
| PR133 source-status-neutral-intro shortcut sweep | `python -m aeread_lab.cli --sweep --task principal_holding_filing_artifact_metadata_source_status_neutral_intro --agents offline:oracle,offline:source_reliability,offline:metadata_source_trusting,offline:metadata_trusting,offline:metadata_naive,offline:artifact_blind,offline:market_value,offline:percent_change,offline:second_best --no-cache` | oracle, source-reliability, and metadata-source-trusting rank first with score regret 0; second-best regret 0.0645161; percent-change regret 0.774194; metadata-trusting, metadata-naive, artifact-blind, and market-value regret 1 |
| PR133 source-status-neutral-intro live repeat | `AEREAD_OPENAI_MAX_OUTPUT_TOKENS=8192 python -m aeread_lab.cli --sweep --task principal_holding_filing_artifact_metadata_source_status_neutral_intro --agents openai:nano,openai:mini,openai:gpt-5.5 --repeat 3 --case multi_artifact_source_status_neutral_intro_conflicting_registry_close_runner_up --no-cache` | `gpt-5.5` is stable-oracle; `mini` is stable metadata-trusting/status-blind with mean regret 1; `nano` is metadata-trusting/status-blind-modal with mean regret 0.666667 |
...
| PR135 focused source-audit ablation tests | `python -m pytest tests/test_tasks.py -k 'source_audit or source_status_neutral_intro or source_status_neutral or source_status_ablation or source_context or source_provenance or validation_process or metadata_history_conflict or history_conflict or metadata_unmarked_conflict or filing_artifact_metadata'` | 56 selected tests passed; source-audit component prompts, sample limits, case-key targeting, and offline geometry are covered |
| PR135 broader C1 filing tests | `python -m pytest tests/test_tasks.py -k 'filing_artifact or filing_trace or principal_holding'` | 92 selected tests passed |
| PR135 source-audit component shortcut sweeps | three `python -m aeread_lab.cli --sweep --task principal_holding_filing_artifact_metadata_source_audit_{primary_only,backfill_only,ratio_only} --agents offline:oracle,offline:source_reliability,offline:metadata_source_trusting,offline:metadata_trusting,offline:metadata_naive,offline:artifact_blind,offline:market_value,offline:percent_change,offline:second_best --no-cache` runs | each component preserves offline geometry: oracle/source-reliability/metadata-source-trusting regret 0; second-best regret 0.0645161; percent-change regret 0.774194; metadata-trusting, metadata-naive, artifact-blind, and market-value regret 1 |
| PR135 primary-only live repeat | `AEREAD_OPENAI_MAX_OUTPUT_TOKENS=8192 python -m aeread_lab.cli --sweep --task principal_holding_filing_artifact_metadata_source_audit_primary_only --agents openai:nano,openai:mini,openai:gpt-5.5 --repeat 3 --case multi_artifact_source_audit_primary_only_conflicting_registry_close_runner_up --no-cache` | `mini` and `gpt-5.5` are stable-oracle; `nano` is unstable-oracle-modal with mean regret 0.333333 and one attributed metadata-trusting/status-blind miss |
| PR135 primary-only live follow-up | `AEREAD_OPENAI_MAX_OUTPUT_TOKENS=8192 python -m aeread_lab.cli --sweep --task principal_holding_filing_artifact_metadata_source_audit_primary_only --agents openai:nano,openai:mini,openai:gpt-5.5 --repeat 6 --case multi_artifact_source_audit_primary_only_conflicting_registry_close_runner_up --no-cache` | `mini` and `gpt-5.5` remain stable-oracle; `nano` is unstable-oracle-modal with mean regret 0.166667, oracle-hit 0.833333, and one attributed metadata-trusting/status-blind miss |
| PR135 backfill-only live repeat | `AEREAD_OPENAI_MAX_OUTPUT_TOKENS=8192 python -m aeread_lab.cli --sweep --task principal_holding_filing_artifact_metadata_source_audit_backfill_only --agents openai:nano,openai:mini,openai:gpt-5.5 --repeat 3 --case multi_artifact_source_audit_backfill_only_conflicting_registry_close_runner_up --no-cache` | `gpt-5.5` is stable-oracle; `nano` is stable metadata-trusting/status-blind with mean regret 1; `mini` is unstable-oracle-modal with one percent-change miss |
| PR135 backfill-only live follow-up | `AEREAD_OPENAI_MAX_OUTPUT_TOKENS=8192 python -m aeread_lab.cli --sweep --task principal_holding_filing_artifact_metadata_source_audit_backfill_only --agents openai:nano,openai:mini,openai:gpt-5.5 --repeat 6 --case multi_artifact_source_audit_backfill_only_conflicting_registry_close_runner_up --no-cache` | `gpt-5.5` remains stable-oracle; `nano` is unstable-oracle-modal with mean regret 0.333333 and metadata-trusting/status-blind misses; `mini` is metadata-trusting/status-blind-modal with mean regret 0.666667 |
| PR135 ratio-only live repeat | `AEREAD_OPENAI_MAX_OUTPUT_TOKENS=8192 python -m aeread_lab.cli --sweep --task principal_holding_filing_artifact_metadata_source_audit_ratio_only --agents openai:nano,openai:mini,openai:gpt-5.5 --repeat 3 --case multi_artifact_source_audit_ratio_only_conflicting_registry_close_runner_up --no-cache` | `mini` and `gpt-5.5` are stable-oracle; `nano` is stable metadata-trusting/status-blind with mean regret 1 |
| PR135 ratio-only live follow-up | `AEREAD_OPENAI_MAX_OUTPUT_TOKENS=8192 python -m aeread_lab.cli --sweep --task principal_holding_filing_artifact_metadata_source_audit_ratio_only --agents openai:nano,openai:mini,openai:gpt-5.5 --repeat 6 --case multi_artifact_source_audit_ratio_only_conflicting_registry_close_runner_up --no-cache` | `mini` and `gpt-5.5` remain stable-oracle; `nano` is metadata-trusting/status-blind-modal with mean regret 0.666667 |
| PR135 full all-task oracle | `python -m aeread_lab.cli --task all --agent offline:oracle --no-cache` | all 107 current task runners execute; oracle path remains clean, including the three source-audit ablations with `n=1`, score regret 0, accuracy 1.00, and oracle margin 0.0645 |
| PR135 full pytest | `python -m pytest` | 339 tests passed |
...
98. PR 122 adds the complementary metadata ablation. It restores a separate
    corporate-action registry while keeping the same conflicting-value rows as
    PR 120. The repeat-3 run is stable-oracle for all aliases; in the repeat-6
    follow-up, `gpt-5.5` remains stable-oracle but `nano` has one artifact-blind
    miss. This pins the top-alias PR 120 miss to missing metadata under a
    value-conflict, while leaving a smaller-model question about reliable
    registry use under repeated sampling.
99. PR 123 tests whether that residual is caused by merely adding realistic
    registry noise. It keeps the same conflicting-value rows and correct
    confirmed split records, but adds stale, unconfirmed, non-split, and
    unmatched corporate-action entries. The offline metadata-naive baseline
    gets pulled to the unconfirmed `sec_stress_c` distractor, but live repeat-3
    across `nano`, `mini`, and `gpt-5.5`, plus repeat-6 on `nano` and `gpt-5.5`,
    are all stable-oracle. This makes the next depth question sharper:
    distractor rows are not enough when the correct confirmed registry entries
    are present; test partial/missing registry coverage, wrong-but-confirmed
    metadata, or real filing traces where the registry and row pattern disagree.
100. PR 124 follows the partial-coverage branch directly. It keeps the
    conflicting-value stress rows and one confirmed split record, but omits the
    other target-period split from the registry, so the oracle must combine
    registry metadata with row-ratio evidence. The offline metadata-only
    baseline collapses to the omitted raw split artifact with full regret. Live
    repeat-3 makes the tier split clear: `mini` and `gpt-5.5` solve, while
    `nano` is unstable-non-oracle-modal. The repeat-6 follow-up preserves the
    result: `gpt-5.5` remains stable-oracle, and `nano` has 0.333 oracle-hit
    with artifact-blind modal choice. The current next question is no longer
...
102. PR 126 removes one scaffold from PR 125: the prompt no longer warns that
    registry rows may conflict with filing rows. The false confirmed split is
    still present and still contradicted by row ratios. Offline baselines are
    unchanged, but live behavior changes sharply: `nano` becomes
    metadata-trusting-modal, `mini` also becomes metadata-trusting-modal in
    repeat-6, and `gpt-5.5` remains stable-oracle. This means the validation
    cue itself was doing important work for smaller aliases. The next C1 depth
    question is whether real-derived filing histories can expose the same
    validation requirement without spelling out either the conflict or the
    intended row-ratio check.
103. PR 127 asks that next question without adding case breadth. It keeps the
     PR 126 false confirmed split and removes the warning, but adds an earlier
     filing period for every issuer so the no-share-change history around
     `sec_stress_d` is visible. The result is negative and useful: `nano` and
     `mini` remain metadata-trusting-modal in repeat-6, while `gpt-5.5` stays
     stable-oracle. Repeated synthetic history alone is therefore not sufficient
     for smaller aliases to reject confirmed but row-inconsistent metadata. The
     next uncovered direction is either real corporate-action source provenance
     or a process-level validation scaffold, not another clean history row.
104. PR 128 tests the process-scaffold branch directly. It keeps PR 127's
     repeated issuer rows and false confirmed split, but adds only a generic
     instruction to compute observed row ratios from filings before applying
     target-period split registry rows. The explicit conflict warning stays
     absent. The result is decisive for this synthetic setup: all three aliases
     are stable-oracle in repeat-6. That means the PR 126/127 failure was not
     missing evidence; it was the absence of a validation procedure cue. The
     next uncovered direction is whether real source provenance, source
     reliability fields, or less explicit corporate-action context can trigger
     the same row/metadata cross-check without handing over the procedure.
105. PR 129 tests the source-provenance branch and the result is negative for
     smaller aliases. It keeps the repeated issuer rows and false confirmed
     split, replaces the procedure with source labels that distinguish direct
     issuer/exchange notices from lower-reliability third-party backfill
     metadata, and still omits the explicit conflict warning. `gpt-5.5` stays
     stable-oracle, but `nano` is stable metadata-trusting and `mini` remains
     metadata-trusting-modal in repeat-6. Source labels by themselves are
     therefore not enough; the next C1 depth step should use real
     corporate-action context or a more natural source-grounding surface rather
     than another synthetic provenance label.
106. PR 130 tests that richer source-grounding surface. It keeps the same
     repeated issuer rows and false confirmed split, but replaces terse source
     labels with natural source packets: issuer/exchange notices for true split
     rows and third-party backfill text without issuer/exchange attachments for
     the false split. The result is worse, not better: all three aliases are
     stable metadata-trusting in repeat-3, and repeat-6 leaves `nano` stable
     metadata-trusting while `mini` and `gpt-5.5` are metadata-trusting-modal.
     Natural context around a `confirmed` false registry row can amplify
     deference to metadata. The next C1 depth step should test removing the
     confirmed-status crutch or using real corporate-action source rows, not
     more prose around a synthetic confirmed false record.
107. PR 131 follows the confirmed-status branch. It keeps PR 130's natural
     source packets, but changes the false third-party split row from
     `status=confirmed` to `status=unverified`. Offline baselines separate the
     mechanisms: oracle, source-reliability, and status-aware metadata all
     solve, while status-blind metadata-naive has full regret. Live repeat-3 and
     repeat-6 make `nano`, `mini`, and `gpt-5.5` stable-oracle. That pins the
...
111. PR 135 runs that ablation map. The result is not a simple "row-ratio cue
     solves it" story. Primary-source recognition is enough for stable
     `mini`/`gpt-5.5` and mostly-correct `nano`, but `nano` still has an
     attributed metadata-trusting/status-blind miss. Backfill skepticism alone
     is weakest: `mini` becomes metadata-trusting/status-blind-modal in
     repeat-6. A light ratio-only cue stabilizes `mini`/`gpt-5.5` but leaves
     `nano` metadata-trusting/status-blind-modal. The full PR 134 source audit
     is still the only tested surface that stabilizes all aliases, so the next
     real build should move the full audit to less synthetic corporate-action
     source rows rather than adding more single-component variants.
