...
| `principal_holding_filing_trace` | real-derived 13F filing-trace C1 task | oracle score regret 0 on one Berkshire 2025Q3->2026Q1 public SEC-derived trace; market-value, low-turnover, max-position, and previous-trend baselines all regret 1; oracle margin 0.7555 | first C1 trace populated from actual SEC 13F rows; depth value is not case count, but the test-derived question: whether the prior flow/turnover shortcut and market-value-drift confusion survive when the trace comes from public filings rather than synthetic notes |
