...
|---|---|---|
| Unit tests | `python -m pytest` | 198 tests passed after PR 89 |
| Full offline oracle task pass | `python -m aeread_lab.cli --task all --agent offline:oracle --no-cache` | all 68 task runners executed; oracle errors/regrets zero or expected near-zero; includes `mechanism_trace_equilibrium_natural` with n=4, score regret 0, revenue miss 0, trace-projection miss 0, equilibrium-blind miss 0, and trace-blind miss 0; scam control `instrument_fires=True`; alignment-tax regret 0; revealed-allocation regret 0 |
