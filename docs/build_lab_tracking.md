...
| 113 | `review/113-real-filing-trace` | real-derived C1 filing trace | public SEC-derived Berkshire 13F rows create one neutralized trace where the material share action beats market-value, low-turnover, max-position, and previous-trend shortcuts |
| 114 | `review/114-raw-filing-trace` | raw-row real-derived C1 filing trace | same public SEC-derived trace, but the model sees only period filing rows and issuer choices instead of a precomputed candidate-change table |
...
| `principal_holding_filing_trace` | real-derived 13F filing-trace C1 task | oracle score regret 0 on one Berkshire 2025Q3->2026Q1 public SEC-derived trace; market-value, low-turnover, max-position, and previous-trend baselines all regret 1; oracle margin 0.7555; first live `nano`/`mini`/`gpt-5.5` repeat sweep is stable-oracle for all aliases | first C1 trace populated from actual SEC 13F rows; depth value is not case count, but the test-derived question. This initial real trace validates the harness but is clean enough for all aliases, so the next depth target is harder real filing ambiguity, not more synthetic variants |
| `principal_holding_filing_trace_raw` | raw-row real-derived 13F filing-trace C1 task | oracle score regret 0 on the same Berkshire public SEC-derived trace; market-value, low-turnover, max-position, and previous-trend baselines all regret 1; oracle margin 0.7555 | removes the precomputed candidate-change scaffold from PR 113 and requires inferring the material share-action issuer from period filing rows. This tests prompt-surface depth on the same evidence before spending on broader real-trace search |
...
| Real-derived C1 filing-trace oracle smoke | `python -m aeread_lab.cli --task principal_holding_filing_trace --agent offline:oracle --no-cache` | n=1, score regret 0, accuracy 1.00, oracle margin 0.7555, and all named shortcut miss rates 0 for the oracle |
| Real-derived C1 filing-trace shortcut sweep | `python -m aeread_lab.cli --sweep --task principal_holding_filing_trace --agents offline:oracle,offline:market_value,offline:low_turnover,offline:max_position,offline:trend --no-cache` | oracle rank 1 with score regret 0; market-value, low-turnover, max-position, and previous-trend shortcut baselines all score regret 1; this is a depth probe for share-action vs filing-value drift, not a new breadth target |
...
| Raw-row real-derived C1 filing-trace keyed sweep | `python -m aeread_lab.cli --sweep --task principal_holding_filing_trace_raw --agents offline:oracle,offline:market_value --repeat 2 --case brk_2026q1_energy_rebalance --no-cache` | oracle is stable-oracle with modal `sec_chevron`; market-value is stable-non-oracle with modal `sec_oxy` and matched reference `market_value` |
...
87. PR 113 begins that real-derived trace path with one public SEC 13F-HR
    ...
    the uncovered direction is whether models separate a material share-driven
    principal action from market-value drift, no-change turnover, large-position
    salience, and previous-quarter trend when the rows are actual filings. The
