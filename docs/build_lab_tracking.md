...
93. PR 117 implements that composite artifact stressor: two natural-note split
    ...
    stochastic across fresh repeats: one run is non-oracle-modal on the
    market-value artifact, while the longer drilldown is oracle-modal with two
    artifact-blind raw-share misses. That narrows the next question to repeat
    reliability and shortcut-mixture attribution, not more clean artifact cases.
