...
|---|---|---|
| Unit tests | `python -m pytest` | 204 tests passed after PR 91 |
| Full offline oracle task pass | `python -m aeread_lab.cli --task all --agent offline:oracle --no-cache` | all 70 task runners executed; oracle errors/regrets zero or expected near-zero; includes `pricing_multi_product_capacity_noisy` with n=4, two-price L1 error 0, revenue gap 0, independent miss 0, and capacity-blind miss 0; includes `market_trace_replenishment_natural` with n=4, constrained terminal-cash regret near 0; includes `mechanism_trace_equilibrium_natural` with n=4, score regret 0; scam control `instrument_fires=True`; alignment-tax regret 0; revealed-allocation regret 0 |
