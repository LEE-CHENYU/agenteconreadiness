...
| 115 | `review/115-filing-runner-up-depth` | targeted real-derived filing trace depth probe | Tiger Global SEC-derived trace adds a runner-up dollar-material diagnostic; live `mini` is unstable and sometimes chooses the second-best action while `nano` and `gpt-5.5` are stable-oracle |
...
| `principal_holding_filing_trace_raw` | raw-row real-derived 13F filing-trace C1 task | same offline dynamic range as `principal_holding_filing_trace`; targeted Tiger raw sweep has second-best regret 0.317432, market-value regret 0.611807, trend regret 0.863578, percent-change regret 0.864072, and low-turnover/max-position regret 1; final Tiger live sweep: `nano`/`gpt-5.5` stable-oracle, `mini` unstable-oracle-modal with one second-best miss in six repeats | removes the precomputed candidate-change scaffold from PR 113 and requires inferring the dollar-material share-action issuer from period filing rows. PR 114 exposed percent-change ambiguity on the Berkshire trace; PR 115 shows the next depth question is repeat stability when a plausible runner-up dollar-material action is close enough for a smaller alias to occasionally choose it |
...
| `AEREAD_OPENAI_MAX_OUTPUT_TOKENS=8192 python -m aeread_lab.cli --sweep --task principal_holding_filing_trace_raw --agents openai:nano,openai:mini,openai:gpt-5.5 --repeat 3 --case tiger_2026q1_megacap_rotation --no-cache` | completed in the review copy; `nano` and `gpt-5.5` are stable-oracle with score regret 0; `mini` is unstable-oracle-modal with mean score regret 0.105811, accuracy 0.666667, and primary range 0.317432 | PR 115 turns the real-filing depth question into a tier/stability signal: the smaller `mini` alias can slip from the oracle `sec_tiger_e` to a plausible runner-up dollar-material reduction while the stronger aliases stay stable on the same neutralized raw rows. |
| `AEREAD_OPENAI_MAX_OUTPUT_TOKENS=8192 python -m aeread_lab.cli --sweep --task principal_holding_filing_trace_raw --agents openai:mini --repeat 6 --case tiger_2026q1_megacap_rotation --no-cache --json` | completed in the review copy; `mini` choice counts are `sec_tiger_e:5`, `sec_tiger_h:1`, parse 1.00, oracle-hit 0.833333, primary mean 0.0529053, and the non-oracle choice matches `second_best` | confirms the live miss is not a percent-change, value-drift, turnover, max-position, or trend collapse; it is a runner-up dollar-material near miss. |
...
91. PR 115 follows that depth criterion with one targeted Tiger Global trace
    ...
    the value is that `mini` is repeat-unstable and its only observed wrong
    answer is the second-best dollar-material action. The next C1 question is
    now narrower again: distinguish true shortcut collapse from runner-up
    action instability, and test whether richer filing artifacts create stable
    non-oracle modes rather than occasional near-miss flips.
