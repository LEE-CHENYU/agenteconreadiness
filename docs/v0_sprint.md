...
|---|---|
| **Team** | Chenyu (lead) + Yuecheng + Zihao + Jingyi (candidate per proposal §9.4) |
...
- **C8 SimplePricingGame** — single-agent, one-shot, **continuous-action** (set price p ∈ [0, P_max]); hidden state = true demand-curve parameters (α*, β*, σ*); **4 policies for TERMS-Bench Eq. 4**: (a) base = declared prior + n=10 noisy historical (price, quantity) sales observations → infer posterior + pick price; (b) belief-substituted (posterior) = Bayes-optimal posterior given directly → pick price; (c) state-revealed = true params revealed → must still compute the revenue-maximizing price itself (no closed-form hint); (d) oracle (π★) = harness's closed-form revenue-maximum price α*/(2β*), kept separate from (c). Eq. 4 = Δ_inf (post−base) + Δ_unc (reveal−post) + Δ_ctrl (oracle−reveal); stochastic revenue payoff
...
| M3 | Layer 3 OracleDecomposable formalization | Zihao | Chenyu (C2 + C8 instantiation) |
...
| M5 | 5-axis taxonomy instrumentation (axes 1+2+3 full; axes 4+5 sketched) | Yuecheng (axes 3-5) + Zihao (axes 1-2) | Chenyu |
...
| E2 | v0 scorer library subset (3 scorers) | Yuecheng | Zihao (correctness review), Chenyu (v0 scaffold) |
| E3 | Generator library + YAML schema (enough for C2 + C8) | Chenyu | Yuecheng (refinement) |
| E4 | Cache layer + CLI | Chenyu + Yuecheng | — |
| E5 | C2 ProductProcurementGame end-to-end (discrete-action) | Chenyu | Zihao (utility-vector oracle formalization), Yuecheng (Q3 4-tier scoring impl) |
| E6 | C8 SimplePricingGame end-to-end (continuous-action) | Chenyu | Zihao (Bayes-optimal pricing argmax oracle derivation), Yuecheng (continuous-action scorer + expected-revenue computation + demand-prior interface) |
| E7 | Leaderboard frontend + diagnostic drill-down page | Chenyu | Yuecheng (review) |
| E8 | Benchmark validation runs (3-5 frontier models) | Chenyu + Yuecheng | — |
| E9 | Documentation (README + contributor onboarding) | Chenyu | Yuecheng (technical content) |

### External (3 workstreams)

| # | Workstream | Primary | Supporting |
|---|---|---|---|
| X1 | Methodology paper §1-§8 writing | Chenyu | All |
| X2 | Andrews / Gonczarowski / Erica external review coordination | Chenyu drafts; Zihao reviews | — |
| X3 | Frontier-lab partner pre-commit + Open Phil grant | Chenyu | — |

**Total: ~13 workstreams** for ~25 person-weeks over 16 weeks.

### Per-person rough allocation

- **Chenyu**: 5-6 workstreams (E3, E5, E6, E7, E9, X1, X2, X3 — leads researcher-facing surface + 2 games + writing + outreach)
...
|---|---|---|---|
| **Phase 0 — trial collaboration** | Weeks 1-4 (2026-06-02 → 2026-06-30) | Each contributor takes 1-2 Phase-0 items from proposal §9.2 (Zihao) / §9.3 (Yuecheng) / §9.4 (Jingyi if engaged); Chenyu drafts §1+§2 in parallel | Phase-0 deliverables for the 4-week checkpoint |
...
- **2026-06-09**: Chenyu ships A4 stability dimension end-to-end
...
- [ ] Chenyu §1+§2 draft (target 2026-06-23)
- [ ] Chenyu A4 stability dimension end-to-end (target 2026-06-09)
