...
**Cooperative Inverse Reinforcement Learning** (Hadfield-Menell, Russell, Abbeel, Dragan, 2016; 1606.03137). Two-player cooperative game: the human knows the reward parameter θ, the robot does not but acts to maximize the *shared* reward, maintaining a Bayesian belief over θ that the human can teach. Two results load-bearing for AERead: (1) the alignment-safe objective is to **maximize the principal's utility under uncertainty about what it is, and stay corrigible** — *point-estimate* reward-maximization is what produces the misaligned (ruthless/over-confident) agent, so the "utility-max vs alignment" tension is really *point-estimate-max* vs alignment, and dissolves under reward uncertainty; (2) "teaching vs demonstrating" (§4.8.2) — an expert's demonstrations can be *suboptimal* for the true θ, the formal root of the inversion problem (observed behavior ≠ preferences). Formal backbone for the configured-principal grade, the Q14 alignment-safe training target, and the C1 13F noisy-oracle caveat (holdings are demonstrations, not θ). Covered pedagogically in CS329H (Truong/Haupt/Koyejo 2025) §4.8; canonical anchor = Hadfield-Menell 2016.
