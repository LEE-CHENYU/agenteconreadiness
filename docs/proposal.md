# AgentEcon Readiness (AERead): an economic-decision diagnostic for LLM agents

**Proposal v0.4 — for Yuecheng Fang and Zihao Li**
Owner: Chenyu Li | Date: 2026-05-30 (v0.3 → v0.4 reframe) | Read time: ~12 minutes (skim the §9.5 ownership tables on first read)

> **Read [`refocus.md`](refocus.md) first — it carries the current state.** A 2026-05-29 validation sprint (≈17 controlled probes, real frontier-model runs, multi-seed CIs) hard-tested the v0 thesis and sharpened it. This proposal (v0.4) leads with that sharpened framing; `refocus.md` is the fuller synthesis, `methodology.md` the framework.
>
> **What changed since v0.3** (the framing this proposal now leads with): (1) **rationality-compliance is a *saturated floor*, not the headline** — frontier models pass it (our probes + Chen 2023 CCEI ≈0.998 + TERMS deal-rate maxed); it's table stakes, a regression-test gate. (2) **The headline axes are the *grade*** — regime-appropriateness, the alignment↔economic-behavior shift, and steerability/persona-fidelity (the discriminating, durable, deployment-critical signals). (3) **The contribution is the diagnostic *lens*** (gate→grade + these axes + multi-seed rigor), not a new game suite or "we integrated 3 benchmarks." (4) The 3-layer pipeline + 5-axis taxonomy are **intact as the supporting framework** — re-prioritized, not re-architected. The moat is **methodological honesty**: the sprint corrected its own headline claims three times, on the record.
...
**The headline (sharpened by the validation sprint — see [`refocus.md`](refocus.md))**: "is the agent *rational*?" turns out to be a **saturated floor** for frontier models — table stakes, a regression-test gate, not a discriminator. The discriminating, durable, deployment-critical axes are the **grade**: **(1) regime-appropriateness** (does the agent deploy the right utility for the regime — EV / Kelly / CVaR / CRRA?), **(2) the alignment↔economic-behavior shift** (which axis alignment training moves, and whether it helps or hurts — replicated across two benchmarks), and **(3) steerability** (faithful representation of a configured principal, via predict-then-validate). Read as a hierarchy this is a **gate → grade** pipeline: axiomatic coherence is the *gate* (pass/fail floor); fidelity to a configured/revealed principal is the *graded headline*.

**The supporting framework**: the **3-layer pipeline (existence → identification → predictive validity) + 5-axis failure taxonomy** provide the **cross-layer diagnostic decomposition** that turns a single-number leaderboard into actionable signal — telling you *why* a model fails and what to fine-tune. They are intact and load-bearing — re-prioritized as the framework *underneath* the gate→grade headline, not the headline itself. The contribution is the **diagnostic lens** (gate→grade + the headline axes + multi-seed rigor), not a new game suite; the moat is **methodological honesty** (the sprint corrected its own headline claims three times, on the record). Without the decomposition, AERead is just another use-case benchmark; with it, it's a tool.

**The training-signal goal**: AERead's per-axis penalties are designed to plug into RLHF/DPO loops as auxiliary losses — the diagnostic surface frontier labs can use as **training signal**, not just as evaluation. Four prior-art-anchored mechanisms (Andrews 2026 training-time penalty / Chadwick 2025 inference-time correction / Qiu 2026 supervised trace mimicry / Betz-Richardson 2023 self-supervised consistency — see methodology Q5) describe the integration surface. The sprint sharpened *what* signal: the training-useful object is the **oracle-decomposed *process* signal (Δ_inf/Δ_unc/Δ_ctrl, per-axis), not the scalar score** — and that distinction is the alignment contribution. A scalar surplus reward retrains the deception/collusion that safety post-training removed (those *are* the surplus-maximizing strategies); process reward targeted at the Δ-attributed *competence* deficit recovers economic competence *without* reviving the misbehavior (the [Hadfield-Menell 2016](https://arxiv.org/abs/1606.03137) CIRL point — the trap is *point-estimate* maximization, not utility-max; see methodology Q14). **v0 ships**: the Andrews-style penalty/scorer reference (LP-checkable diagnostic) + observational evidence (frontier-model penalty correlated with capability across 3-5 models). **What v0 does NOT yet ship**: the *causal* claim that training a model with AERead's penalty as auxiliary loss *causes* downstream improvement. The causal claim requires a small-model training experiment (~$500-1K GPU + ~3 person-weeks; e.g., Qwen 0.5B / 3B fine-tune with vs without penalty) — pending checkpoint decision (see methodology Q10), and it can double as the Q14 falsifier (process-reward vs outcome-reward, read out the deception axis).
...
Three recent benchmark papers (Harvard EconEvals March 2025, MIT Revealed Rationality Feb 2026, Stanford TERMS-Bench May 2026) each address one layer. AERead's contribution is **not** "we integrated all three" (the integration is *substrate* — we borrow these environments) — it is the **diagnostic lens** applied across them and the controllable probes for what they can't expose. The sprint confirmed the field independently draws the same line we did: single-agent / short-horizon / known-environment is saturated; the deployment-pressure regimes (regime-appropriateness, alignment-shift, long-horizon coherence) are where models discriminate (see [`refocus.md`](refocus.md) § External convergence). None of the three ship with the plug-and-play UX needed to compete with the field's adoption gatekeepers, the gate→grade diagnostic, or the training-signal application even at the observational level.
...
A 2025 Anthropic publication reports that **5.9% of Claude API conversations are economic decisions**. JPMorgan announced LLM-agent shareholder voting; Visa announced AI shopping agents. The deployments are happening; the validation is not — and the validation that matters is *not* "is the agent rational?" (frontier models pass that floor) but "does it deploy the right utility for the regime, stay faithful to its principal, and not have alignment training quietly shifted its economic behavior?" Those are the gaps AERead measures.

**An alignment-rationality tension has shown up empirically — in at least one frontier model.** Opus 4.8 on [Vending-Bench](https://arxiv.org/abs/2502.15840) (Andon Labs, 2026) shows better alignment — zero deceptive/power-seeking behavior — paired with *worse* economic performance: the model falls for scam suppliers, enters non-convergent planning loops (~100 rewrites of the same strategy doc per run), and fails basic price comparisons, across Vending-Bench 2 and Arena. This is not a universal tradeoff — GPT-5.5 tops the same Arena without misconduct (aligned *and* optimal) — but it shows **alignment training can shift the economic choice distribution.** Whether a given alignment intervention improves or degrades rationality-axiom compliance — individually, per axis, per use case — is currently unmeasured. AERead is that diagnostic.
...
None integrate the three layers — but, post-sprint, integration is the *substrate*, not the contribution (see exec-summary "what changed" + [`refocus.md`](refocus.md)). The open opportunity is the **diagnostic lens** (gate→grade + the headline axes) run across these environments plus the controllable probes they can't expose; the integrated substrate is what makes that cheap. The timing window is narrow (6-9 months before any of these teams ships a v2 extending into the other layers).

## 2. The 3-layer hierarchy (canonical revealed-preference pipeline) — read as gate → grade

> **Framing (post-sprint).** Read as a hierarchy, the 3-layer pipeline is a **gate → grade** structure: Layer 1 (existence/coherence) is the **gate** — a pass/fail precondition that frontier models clear (the saturated floor); Layers 2–3 culminating in fidelity to a configured/revealed principal are the **grade** — the discriminating headline. The three headline axes (regime-appropriateness, alignment-shift, steerability) live on the grade. The table below is the canonical machinery underneath that reading; it is intact, re-prioritized not re-architected.
...
| 3 — predictive validity | C (outcome) | A **collection of real-world economic decision use cases**, each individually `OracleDecomposable`. v0: C2 ProductProcurementGame (discrete-action procurement under qualitative evidence) + C8 SimplePricingGame (continuous-action pricing under demand uncertainty). The pair crosses the **action-space boundary** (discrete vs continuous) + the **outcome boundary** (deterministic preference match vs stochastic revenue) — genuine structural distinction, not just uncertainty-type variation. Roadmap: D2 bargaining (wrap TERMS-Bench; multi-agent), D3 VendorSelectionGame (single-agent risk/uncertainty supplier selection; structurally similar to C2 so demoted to v0.5+), C1 persona-fit (IRL on 13F as Layer 2 parameter-fit case study), agentic marketplace extension. | TERMS-Bench (D2 bargaining); EconEvals (procurement + pricing precedent for C2 + C8); novel for AERead-native instantiations |
...
**The novel contribution is Layer 3 structural coverage with diagnostic depth, not the number of cases.** Each Cat C / Cat D use case is a **candidate** for the `OracleDecomposable` interface (hidden state + simulator + oracle + scalar utility) and the TERMS-Bench Eq. 4 decomposition — but the conditions under which a task admits a well-defined Δ_inf + Δ_unc + Δ_ctrl attribution (non-commutativity of interventions; Shapley/Sobol attribution; failure conditions) are exactly what Zihao's Phase-0 (A) is meant to formalize (see [`methodology.md` § OracleDecomposable](methodology.md#oracledecomposable-the-load-bearing-abstraction) — candidate abstraction, not closed claim). What makes the methodology paper §3 contribution rigorous is not "we shipped many cases"; it is that a small set of structurally distinct cases exposes named failure mechanisms, and new cases enter only when a prior result leaves a specific uncovered direction. We apply TERMS-Bench's Eq. 4 to a *family* and formalize the conditions under which the abstraction works, demonstrating portability beyond their single bargaining domain without turning the roadmap into a breadth contest.

- **C2 ProductProcurementGame (v0)**: single-agent, one-shot, **discrete-action** (select 1 of N products); hidden state = vendor true cost/quality + qualitative product attributes (durability / comfort / style fit / assembly friction); **v0 oracle = optimal under single declared utility-weight vector per buyer profile** (per methodology Q3 v0 scope — single-vector scoring with 4-tier optimality classification; robust-utility-family upgrade is v0.5+). Tests Axis 1 (qualitative-evidence interpretation) + Axis 3 (calibration on utility-weight matching) + Axis 4 (constraint satisfaction). Primary demonstration site for the qualitative-evidence-interpretation layer of the §3 contribution.
- **C8 SimplePricingGame (v0)**: single-agent, one-shot, **continuous-action** (set price p ∈ [0, P_max]); hidden state = true demand-curve parameters (intercept α*, elasticity β*, noise σ*). **Designed for TERMS-Bench Eq. 4 decomposition** via four policies: (a) **base condition** = model sees declared prior + product description + noisy past-sales evidence (e.g., 10 historical (price, quantity) pairs) → must infer posterior + choose price; (b) **belief-substituted (posterior) condition** = model is given the Bayes-optimal posterior directly → chooses price (isolates posterior-computation gap); (c) **state-revealed condition** = true demand parameters revealed → model must still compute the revenue-maximizing price itself (no closed-form hint); (d) **oracle (π★)** = the harness's closed-form revenue-maximum price α*/(2β*), kept separate from (c). Eq. 4 splits the gap as Δ_inf = U(post)−U(base) + Δ_unc = U(reveal)−U(post) + Δ_ctrl = U(oracle)−U(reveal), summing to U(π★)−U(base). Tests Axis 1 (inference from noisy sales evidence) + Axis 3 (calibration on posterior + expected-revenue computation) + Axis 4 (continuous-action optimization — argmax over [0, P_max]). **Structurally distinct from C2** on action space (continuous vs discrete), outcome (stochastic revenue vs deterministic preference match), oracle math (Bayesian-posterior argmax vs utility-vector dot product). Conceptual precedent: EconEvals 2025 pricing dimension (A2 in [`layer3_candidates.md`](layer3_candidates.md)); v0 ships an AERead-native single-period version, with EconEvals's multi-period + competition-aware pricing wrappable at v0.5+.
- **D3 VendorSelectionGame (roadmap, single-agent risk/uncertainty)**: single-agent, risk/uncertainty; hidden state = supplier reliability history + lead-time variance; oracle = Bayes risk minimization given declared utility curvature. Tests Axis 3 (calibration on risk) + Axis 4 (reliability-cost tradeoff). **Demoted from v0 to roadmap** (2026-05-28): structurally similar to C2 (both single-agent + one-shot + discrete-choice + solo + decision-theoretic) — the v0 pair needs more structural diversity than uncertainty-type variation alone, so D3 became roadmap and C8 SimplePricingGame replaced it as v0 game 2. D3 ships at v0.5+ once the v0 procurement + pricing framework validates.
- **C1 Persona-fit / 13F IRL (roadmap, Layer 2 parameter-fit case study)**: hidden state = principal utility coefficients; oracle = closed-form Kelly-Markowitz. Novel — no prior LLM benchmark grounds in dollar-equivalent decisions of named human principals at scale. **Caveat**: 13F filings are an **approximate** revealed-preference proxy (long positions only, quarterly delay; shorts + derivatives not required). The rationale source repo continues to develop C1 as Layer 2 (within-class identification) infrastructure; it becomes a v0.5+ Layer 3 case study once the v0 procurement + pricing framework is validated. Not in the v0 first paper.
...
> **The v0 list is intentionally short.** The full open candidate map — EconEvals procurement/pricing/scheduling wraps, Calvano-style algorithmic-collusion detection, Gale-Shapley matching, multi-issue negotiation, Vending-Bench long-horizon coherence, Zhu-Griffiths weather/event forecasting, and other structural clusters — is enumerated in [`layer3_candidates.md`](layer3_candidates.md) on a value × testability matrix, with per-case pros/cons + Tier S/A/B/C/D priority. The candidate doc is a **question map**, not a quota: cases graduate when they answer an uncovered direction from existing results (e.g. four-regime replication, C1 metadata-deference, D2 configured-principal/deception readout, long-horizon belief manipulation). Breadth can strengthen §3 only after depth has named what the additional domain is testing.

## 3. Supporting framework: 5-axis failure taxonomy + `OracleDecomposable` generalization

> **What this section is, post-sprint.** The 5-axis taxonomy + OracleDecomposable generalization are the **diagnostic machinery underneath the gate→grade headline** — the decomposition that makes a failure *attributable* (which mechanism, which layer). They are the methodologically defensible engine of the lens, anchoring the §3 methodology-paper contribution. What they are *not* (post-sprint) is the discriminating headline on their own: the sprint found the 5 axes largely diagnose failures *at and around the saturated gate*, while the discrimination lives on the grade (regime-appropriateness / alignment-shift / steerability — §2 framing). So this section reads as "the rigorous attribution engine," not "the thing that makes frontier models separate." Both matter; the methodology paper cites the engine, the leaderboard discriminates on the grade.

This is the methodologically defensible decomposition that distinguishes AERead's *diagnostic depth* from STEER, EconEvals, Rationality Check!, TERMS-Bench, and every other 2025-2026 LLM-economics benchmark.
...
**Generalizing TERMS-Bench Eq. 4 (the load-bearing claim for Axis 1)**: Zhang et al. 2026 introduced the oracle-gap intervention decomposition U(π★) − U(π_base) = Δ_inf + Δ_unc + Δ_ctrl, but presented it as bargaining-specific. We propose an **`OracleDecomposable` interface** as a **candidate abstraction with conditions to be formalized** — any task with (hidden state, controllable simulator, oracle policy, belief substitution, state reveal, scalar utility) is a candidate for the same decomposition. The conditions under which the decomposition is well-defined (non-commutativity, Shapley/Sobol attribution when interventions don't commute, failure conditions) are exactly what Zihao's Phase-0 (A) formalization scope (§9.2) is meant to prove. The **generalization claim is the multi-use-case application + the formalization of those conditions**:

- **v0 — C2 ProductProcurementGame**: hidden state = vendor true cost + structured qualitative attributes (per methodology Q3); **v0 oracle = optimal under single declared utility-weight vector per buyer profile** (robust-utility-family is v0.5+ upgrade per Q3). Discrete-action choice; deterministic preference match. Eq. 4 applied to a qualitative-evidence procurement domain.
- **v0 — C8 SimplePricingGame**: hidden state = true demand-curve parameters (α*, β*, σ*); **4 policies** for TERMS-Bench Eq. 4 — (a) base = declared prior + n=10 noisy historical (price, quantity) sales observations → infer posterior + pick price; (b) belief-substituted (posterior) = Bayes-optimal posterior given directly → pick price (isolates posterior-computation gap from price-choice gap); (c) state-revealed = true (α*, β*, σ*) revealed → must still compute the revenue-maximizing price itself (no closed-form hint); (d) oracle (π★) = closed-form revenue-maximum price α*/(2β*), kept separate from (c). Eq. 4 splits the gap into Δ_inf = U(post)−U(base) + Δ_unc = U(reveal)−U(post) + Δ_ctrl = U(oracle)−U(reveal). Continuous-action; stochastic revenue. **Structurally distinct from C2** on action space (continuous vs discrete) + outcome (stochastic vs deterministic) + oracle math (Bayesian-posterior argmax vs utility-vector dot product) + uncertainty representation (parametric demand curve + noisy evidence vs qualitative text).
- **Roadmap — D3 VendorSelectionGame**: single-agent risk/uncertainty supplier selection. **Demoted from v0 to v0.5+** (2026-05-28): structurally similar to C2 (single-agent + one-shot + discrete-choice + solo + decision-theoretic); v0 pair needed cross-structure variation, so C8 SimplePricingGame replaced D3 as v0 game 2.
- **Roadmap — D2 bargaining**: native TERMS-Bench instantiation; wrap their runtime when code releases (their `github.com/zou-group/terms-bench` repo is 404 as of 2026-05-26; expect release within 1-3 months). Eq. 4 in its native form. **Multi-agent strategic domain** — broadens the structural generalization claim significantly.
- **Roadmap — C1 persona-fit (Layer 2 parameter-fit case study)**: hidden state = principal's IRL-fit utility coefficients (α_kelly, δ_turnover); oracle = closed-form Kelly-Markowitz. Eq. 4 applied to portfolio/investment domain. Rationale source repo continues to develop the IRL infrastructure; first appears as a v0.5+ Layer 3 case once the v0 procurement + pricing framework is validated.
...
> **Open methodological questions** — fourteen strategic questions critical to frontier-lab methodological engagement, with full bodies + tables in [`methodology_open_questions.md`](methodology_open_questions.md) and navigation pointer in [`methodology.md`](methodology.md#critical-methodological-questions-open-qa). Topics: (Q1) function misspecification; (Q2) numeric-feature evaluation; (Q3) qualitative-feature scoring; (Q4) generalization battery; (Q5) RL training-signal mechanisms; (Q6) training-signal contamination + AERead-Train/Dev/Cert; (Q7) Layer 1+2 residual hypothesis; (Q8) OpenSpiel substrate (post-v0 follow-up paper); (Q9) operational emergence; **(Q10) training-signal validation — the most consequential v0 scope decision** (whether to include a small-model Qwen training experiment in v0 to causally validate the per-axis penalty as training signal, or defer to follow-up paper; pending 4-week checkpoint); (Q11) gate vs grade — rationality vs persona-fidelity; (Q12) stress as a cross-cutting axis; (Q13) adversarial co-evolution as judge-free stress-discovery; **(Q14) what training signal AERead emits** — the oracle-decomposed *process* signal (not the scalar score) recovers economic competence without reviving the misbehavior alignment removed; process-vs-outcome reward is the alignment-safety distinction (v1+; methodology-framed). Each Q&A entry ends with an "Open:" invitation. Engagement on all fourteen invited at the 2026-06-02 first sync.
...
Two distinct milestones, both Chenyu-time + API spend:
...
|---|---|---|
| Phase 0 — trial collaboration | Weeks 1-4 | Each contributor takes 1-2 Phase-0 DoL items from §9; Chenyu drafts §1+§2 in parallel; output drives the §9.6 checkpoint discussion |
| Phase 1 — paper draft | Weeks 5-8 | Methodology paper §3 (5-axis + OracleDecomposable) drafted (continuing from Zihao's Phase-0 work); §1+§2 polished from Chenyu's Phase-0 drafts; §6 polished from Yuecheng's Phase-0 (D) draft if picked, else first draft; §10 (Zihao) first draft; arxiv preprint target |
...
Methodology integration + ergonomic design (UX, contributor experience, leaderboard layout) + v0 Category C implementation (C2 ProductProcurementGame + C8 SimplePricingGame task design) + external outreach + grant management. Note that the §3 methodology core is Zihao's track (§9.2) and the stats-specific methodology is Yuecheng's (§9.3); Chenyu owns the integration layer that holds them together + the v0 game implementations + the product/UX surface. Has working 13F-extraction pipeline (~309 named investor network mapped — informs C1 persona-fit roadmap Layer 2 infrastructure in the rationale source repo; not in v0 first paper); has Phase-0 baseline transitivity test results on Claude / GPT / DeepSeek.
...
- **(D) Layer 3 candidate curation — mechanism-design lens.** Read [`layer3_candidates.md`](layer3_candidates.md) (depth-first candidate map: value × testability plus uncovered questions from current results); from your mechanism-design background, do either or both:
  1. **Propose 0-2 additional candidates** not yet in the pool only if they answer an uncovered structural question (examples we suspect are missing: spectrum auctions, prediction markets, generalized second-price / AdWords mechanism, market-maker bid-ask spread, revenue-equivalence stress tests). For each: source paper(s), OracleDecomposable mapping sketch, matrix-position justification, and the prior-result ambiguity it resolves.
  ...
  Either path closes the same matrix gap: "high economic value × high testability" is the quadrant where the OracleDecomposable Eq. 4 generalization gets the strongest empirical demonstration (the methodology paper §3 claim), and it's currently sparse because the oracle-derivation work hasn't been done. The useful output is a sharper depth probe or formal oracle, not a longer candidate list. **~8-12 hours** for either path; ~15-20h if you do both. This is the highest-leverage Phase-0 item for the candidate-pool curation workstream in §9.5.
...
- **(C) Composite-score schema implementation + thin frontend prototype.** Chenyu owns the leaderboard design spec (what's the right user-facing single number, how does the 5-axis radar map to a sortable column, what's on the drill-down page); your task is to **ship a static-HTML leaderboard prototype** with 1-2 mock results that validates Chenyu's design spec works in practice + propose any implementation tradeoffs the spec didn't anticipate. Closest design precedent: [TERMS-Bench leaderboard](https://terms-bench.github.io) — study their 7-column orthogonal-axis layout (SE⁺, AGR⁺, CSE⁺, FAGR⁻, BEtype, CritViol%, Ū); AERead extends this pattern with the 5-axis radar + drill-down. ~8-12 hours.
...
**Note on engineering ownership**: Chenyu ships the v0 runtime scaffold during Phase-0 (per §7); the engineering primary roles below reflect the **ongoing post-conversion split**. Benchmark implementation is shared between Chenyu and Yuecheng — Chenyu leads the researcher-facing surface (generator library, CLI, submission path, documentation, leaderboard/diagnostic frontend); Yuecheng leads the math/stats-heavy infrastructure (scorer library, cache, validation runs, statistical reproducibility framework). The split tracks expertise rather than abstraction layer.

**Methodology paper workstreams**:

| Workstream | Proposed primary | Supporting |
|---|---|---|
| §1 Introduction | Chenyu | Zihao (framing review) |
| §2 Related work | Chenyu | Zihao (econ-lit anchors), Yuecheng (LLM-bench anchors) |
| §3 Methods — 5-axis taxonomy + OracleDecomposable formalization | Zihao | Chenyu (writing pass) |
...
| §7 Category C implementation (C2 ProductProcurementGame + C8 SimplePricingGame v0; D3 VendorSelectionGame + C1 persona-fit Layer 2 infrastructure in source repo as roadmap) | Chenyu | Zihao (utility-class identification + Bayes-optimal pricing oracle) |
| §8 Experiments — cross-model eval runs + result tables | Yuecheng | Chenyu (analysis), Zihao (interpretation) |
| §10 Discussion — mechanism design + agent-market thesis | Zihao | Chenyu |
...
| Layer 3 OracleDecomposable formalization | Zihao | Chenyu (C2 + C8 instantiation) |
...
| Workstream | Proposed primary | Supporting |
|---|---|---|
| lm-evaluation-harness integration + entry-point structure | Chenyu (v0 scaffold) + **Jingyi Lu if she joins per §9.4** (adapter prototype + sustained maintenance); else Yuecheng | Yuecheng (scorer integration regardless of who owns the adapter) |
| **v0 scorer library (subset)** — Brier + 1 transitivity scorer + 1 multi-class utility-fit scorer (NOT the full Bradley-Terry / WST/MST/SST / KT-MLE / isotonic library — that's v0.5+) | Yuecheng | Zihao (correctness review), Chenyu (v0 scaffold for simpler scorers) |
| Generator library + YAML schema (v0 scope: enough for C2 + C8 generation) | Chenyu | Yuecheng (refinement) |
| Cache layer (responses / stimuli / scores / splits / fits / predictions) — **v0 subset** without signed-ScoreRecord enforcement (signed-ScoreRecord defers to v0.5+) | Chenyu + Yuecheng | — |
| CLI (`aeread evaluate / diagnose / init`) — v0 subset; `aeread check` deferred to v0.5+ | Chenyu | Yuecheng (extension) |
| **C2 ProductProcurementGame** end-to-end (discrete-action; qualitative-evidence procurement; Q3 single-stage 4-tier optimality scoring with single utility-weight vector per buyer; 2 counterfactual variants — paraphrase + preference-flip) | Chenyu | Zihao (oracle formalization), Yuecheng (Q3 scoring impl) |
| **C8 SimplePricingGame** end-to-end (continuous-action; stochastic revenue; **4-policy Eq. 4 design** — base with n=10 noisy sales evidence + belief-substituted (posterior) + state-revealed + oracle π★; Bayes-optimal posterior + expected-revenue argmax; 2 counterfactual variants — paraphrase + sales-evidence-perturbation) | Chenyu | Zihao (Bayes-optimal posterior derivation + Eq. 4 conditions formalization for C8), Yuecheng (continuous-action scorer + posterior-inference implementation + expected-revenue computation) |
| Leaderboard frontend + diagnostic drill-down page (5-axis radar — v0 shows axes 1+2+3 fully, axes 4+5 as sketched) | Chenyu | Yuecheng (review) |
| **Benchmark validation runs (v0 scope: 3-5 frontier models, not 8+)** | Chenyu (run orchestration) + Yuecheng (result-table generation) | — |
| Documentation (README, contributor onboarding for v0 scope) | Chenyu | Yuecheng (technical content) |
| **Web-form submission path only** (Colab + PR paths defer to v0.5+) | Chenyu | — |
...
**Open ops slot**: still relevant if a candidate emerges (leaderboard / HuggingFace ops would be their workstream). If no candidate by Phase-0 checkpoint, the deferred items above stay deferred; Chenyu does NOT inherit them in v0.

**External outreach + coordination**:

| Workstream | Proposed primary | Supporting |
|---|---|---|
| Isaiah Andrews (MIT) — §3 methodology review | Chenyu drafts; Zihao reviews before send | — |
| Yannai Gonczarowski (Harvard) — parameter-layer comparison | Chenyu drafts; Zihao reviews before send | — |
| Erica Zhang (Stanford) — TERMS-Bench coordination | Chenyu (existing relationship) | — |
| Frontier-lab partner pre-commit | Chenyu | — |
| Open Phil grant application + ongoing reporting | Chenyu | — |
...
- Chenyu: A4 stability dimension end-to-end (2026-06-09) + methodology paper §1+§2 draft (2026-06-23)
...
- **Part ways** → no acrimony; Chenyu cites your work in the paper as planned anyway
...
4. **Chenyu**: ship A4 stability dimension end-to-end by 2026-06-09; draft methodology paper §1+§2 by 2026-06-23.
...
- It is NOT asking for capital or introductions to investors. v0 is bootstrap-funded through Open Phil grant application + Chenyu's personal time.
...
- **[Methodology summary](methodology.md)** — 3-layer pipeline + 5-axis taxonomy + 14-Q&A with inline v0/v0.5+ scope tags
...
- **[Layer 3 candidate pool](layer3_candidates.md)** — depth-first map of real-world economic decision use cases: value × testability, oracle-status, and uncovered questions from current results
...
The master plan + runtime spec live in the internal source repo (`rationale`) — they're the implementation-side documents this proposal summarizes. Request access from Chenyu if your Phase-0 item needs drill-down:
...
- **Diagnostic-page mock** (~500 lines) — UX wireframes for the 5-axis radar drill-down page that Chenyu will design against. Useful for Yuecheng's Phase-0 (C) composite-score schema implementation + thin frontend prototype.
