...
**The novel contribution is the Layer 3 collection-of-use-cases framing**, not any single use case. Each Cat C / Cat D use case is a **candidate** for the `OracleDecomposable` interface (hidden state + simulator + oracle + scalar utility) and the TERMS-Bench Eq. 4 decomposition — but the conditions under which a task admits a well-defined Δ_inf + Δ_unc + Δ_ctrl attribution (non-commutativity of interventions; Shapley/Sobol attribution; failure conditions) are exactly what Zihao's Phase-0 (A) is meant to formalize (see [`methodology.md` § OracleDecomposable](methodology.md#oracledecomposable-the-load-bearing-abstraction) — candidate abstraction, not closed claim). What makes the methodology paper §3 contribution rigorous: we don't just apply TERMS-Bench's Eq. 4 to one new task; we apply it to a *family* + formalize the conditions under which the abstraction works, demonstrating portability beyond their single bargaining domain.
