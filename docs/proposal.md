...
- **v0 — C8 SimplePricingGame**: hidden state = demand-curve parameters (intercept α, elasticity β, noise σ); model receives declared demand prior + chooses price ∈ [0, P_max]; oracle = Bayes-optimal expected-revenue argmax. Continuous-action; stochastic revenue. Eq. 4 applied to a pricing-under-demand-uncertainty domain. **Structurally distinct from C2** on action space (continuous vs discrete) + outcome (stochastic vs deterministic) + oracle math (argmax of expected revenue vs utility-vector dot product) + uncertainty representation (parametric demand curve vs qualitative text).
...
| **C8 SimplePricingGame** end-to-end (continuous-action; stochastic revenue; Bayes-optimal argmax oracle under declared demand prior; 2 counterfactual variants — paraphrase + demand-prior-perturbation) | Cheney | Zihao (Bayes-optimal pricing oracle derivation), Yuecheng (continuous-action scorer + expected-revenue computation) |
