...
- **C8 SimplePricingGame (v0)**: single-agent, one-shot, **continuous-action** (set price p ∈ [0, P_max]); hidden state = demand-curve parameters (intercept α, price elasticity β, noise σ) — the model is given a declared demand prior (e.g., "demand is approximately 100 − 5p with noise σ=10") and must choose a revenue-maximizing price; oracle = Bayes-optimal price = argmax_p E[p × D(p)] under the declared prior. Tests Axis 3 (calibration on expected-revenue computation) + Axis 4 (continuous-action optimization — can the model find argmax over a continuous space?) + Axis 1 (information processing from demand prior). **Structurally distinct from C2** on action space (continuous vs discrete), outcome (stochastic revenue vs deterministic preference match), oracle math (optimization argmax vs utility-vector dot product), uncertainty representation (parametric demand curve vs qualitative text). Conceptual precedent: EconEvals 2025 pricing dimension (A2 in [`layer3_candidates.md`](layer3_candidates.md)); v0 ships an AERead-native simple version (single-period, no competition), with EconEvals's multi-period + competition-aware pricing wrappable at v0.5+.
