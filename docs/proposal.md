...
- **C8 SimplePricingGame (v0)**: single-agent, one-shot, **continuous-action** (set price p ∈ [0, P_max]); hidden state = true demand-curve parameters (intercept α*, elasticity β*, noise σ*). **Designed for TERMS-Bench Eq. 4 decomposition** via three conditions: (a) **base condition** = model sees declared prior + product description + noisy past-sales evidence (e.g., 10 historical (price, quantity) pairs) → must infer posterior + choose price; (b) **belief-substituted condition** = model is given the Bayes-optimal posterior directly → chooses price (isolates posterior-computation gap); (c) **state-revealed condition** = true demand parameters revealed → chooses oracle price. Eq. 4 splits the gap as Δ_inf (base→revealed) + Δ_unc (base→posterior-substituted) + Δ_ctrl (posterior→revealed). Oracle = Bayes-optimal price = argmax_p E[p × D(p)] under the posterior. Tests Axis 1 (inference from noisy sales evidence) + Axis 3 (calibration on posterior + expected-revenue computation) + Axis 4 (continuous-action optimization — argmax over [0, P_max]). **Structurally distinct from C2** on action space (continuous vs discrete), outcome (stochastic revenue vs deterministic preference match), oracle math (Bayesian-posterior argmax vs utility-vector dot product). Conceptual precedent: EconEvals 2025 pricing dimension (A2 in [`layer3_candidates.md`](layer3_candidates.md)); v0 ships an AERead-native single-period version, with EconEvals's multi-period + competition-aware pricing wrappable at v0.5+.
...
- **v0 — C8 SimplePricingGame**: hidden state = true demand-curve parameters (α*, β*, σ*); **3 conditions** for TERMS-Bench Eq. 4 — (a) base = declared prior + n=10 noisy historical (price, quantity) sales observations → infer posterior + pick price; (b) belief-substituted = Bayes-optimal posterior given directly → pick price (isolates posterior-computation gap from price-choice gap); (c) state-revealed = true (α*, β*, σ*) revealed → pick oracle price = argmax_p p × (α* − β*·p). Eq. 4 splits the gap into Δ_inf (state-revealed − base) + Δ_unc (belief-substituted − base) + Δ_ctrl (state-revealed − belief-substituted). Continuous-action; stochastic revenue. **Structurally distinct from C2** on action space (continuous vs discrete) + outcome (stochastic vs deterministic) + oracle math (Bayesian-posterior argmax vs utility-vector dot product) + uncertainty representation (parametric demand curve + noisy evidence vs qualitative text).
...
| §7 Category C implementation (C2 ProductProcurementGame + C8 SimplePricingGame v0; D3 VendorSelectionGame + C1 persona-fit Layer 2 infrastructure in source repo as roadmap) | Cheney | Zihao (utility-class identification + Bayes-optimal pricing oracle) |
