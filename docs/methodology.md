...
**The EV-default is a liability (regime-blindness).** The framing-stable default is *risk-neutral EV-maximization*, which is optimal only in a narrow box (single-shot/additive, no ruin boundary, finite mean, risk-neutral principal, known probabilities, non-adversarial). Outside it a different utility is *objectively* better: Kelly/log-utility under compounding, drawdown/CVaR under a ruin boundary, CRRA matched to a risk-averse principal, maxmin-EU under ambiguity. So **steering off the EV-default toward the regime-appropriate utility is *correctness*, not just preference-matching** — a stronger leg for the grade. This makes framing-invariance double-edged: insensitivity to irrelevant *surface* reframing is good, but insensitivity to a *substantive* regime change (single-shot ↔ repeated-compounding, which should flip EV→Kelly) is **regime-blindness**. Preliminary probe (same favorable gamble, single-shot vs repeated-N-compounding, no Kelly hint): one frontier model EV-maxed the arithmetic mean into ~79% ruin at the moderate-horizon (N=3) transition before switching to Kelly at long horizons; the other applied Kelly uniformly (under-betting the single-shot, the safe direction). The failure is real, surfaces at the *transition*, and the endpoint summary metric hides it — the full bet-fraction-vs-horizon curve is load-bearing (same lesson as multi-seed: summaries hide failure modes).
