...
| **Q5** | RL environment + training-signal mechanisms | 4 prior-art-anchored mechanisms (Andrews/Chadwick/Qiu/Betz-Richardson) all *described* in paper; **v0 ships Andrews-style penalty/scorer reference only** (LP-checkable diagnostic surface; auxiliary-loss fine-tuning pipeline NOT in v0 — labs wire into their own training); other 3 mechanism implementations deferred to follow-up paper. Factorial intervention design as training-data composition guide. |
...
|---|---|---|---|---|
| 1 | Training-time penalty (label-free axiom loss) | [Andrews 2026](papers/pdfs/andrews_2026_revealed_rationality.pdf) | de Finetti / Afriat / SARSEU penalties as auxiliary loss; zero iff the data are rationalizable | **v0 ships Andrews-style penalty/scorer reference** — the LP-checkable diagnostic surface (compute the penalty on any (prompt, response) pair). The auxiliary-loss fine-tuning pipeline (wire penalty into the training loop) is NOT in v0 — labs can wire AERead's diagnostic into their own training. Full end-to-end fine-tuning integration defers to follow-up paper. |
