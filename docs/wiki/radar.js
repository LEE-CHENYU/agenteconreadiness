...
(function(){
  "use strict";
  var DATA={"claude-opus-4.8": {"vals": {"eqc": 0.1389, "kuhn": 0.278, "intertemporal": 0.024, "cross_modality": 0.0, "allais": 0.0}, "mean": 0.088, "light": 0}, "claude-sonnet-4.6": {"vals": {"eqc": 0.1306, "kuhn": 0.111, "intertemporal": 0.048, "cross_modality": 0.0, "allais": 0.0}, "mean": 0.058, "light": 0}, "claude-opus-4.5": {"vals": {"eqc": 0.0926, "kuhn": 0.111, "intertemporal": 0.645, "cross_modality": 0.0, "allais": 0.0}, "mean": 0.17, "light": 0}, "grok-4.3": {"vals": {"eqc": 0.0, "kuhn": 0.111, "intertemporal": 0.0, "cross_modality": 0.0, "allais": 0.0}, "mean": 0.022, "light": 0}, "gpt-5.5": {"vals": {"eqc": 0.0, "kuhn": 0.278, "intertemporal": 0.0, "cross_modality": 0.0, "allais": 0.0}, "mean": 0.056, "light": 1}, "gemini-3.5-flash": {"vals": {"eqc": 0.1396, "kuhn": 0.278, "intertemporal": 0.264, "cross_modality": 0.0, "allais": 0.0}, "mean": 0.136, "light": 1}, "gpt-5.1": {"vals": {"eqc": 0.1285, "kuhn": 0.444, "intertemporal": 0.026, "cross_modality": 0.0, "allais": 0.625}, "mean": 0.245, "light": 0}, "gemini-2.5-flash": {"vals": {"eqc": 0.0768, "kuhn": 0.111, "intertemporal": 0.72, "cross_modality": 0.75, "allais": 0.0}, "mean": 0.332, "light": 0}}, ORDER=["grok-4.3", "gpt-5.5", "claude-sonnet-4.6", "claude-opus-4.8", "gemini-3.5-flash", "claude-opus-4.5", "gpt-5.1", "gemini-2.5-flash"], COLORS={"claude-sonnet-4.6": "#4daf4a", "grok-4.3": "#a65628", "gpt-5.5": "#377eb8", "claude-opus-4.8": "#984ea3", "claude-opus-4.5": "#f781bf", "gemini-3.5-flash": "#ff7f00", "gpt-5.1": "#999999", "gemini-2.5-flash": "#e41a1c"}, DEFAULT=["gpt-5.1","gpt-5.5","gemini-2.5-flash","gemini-3.5-flash"];
  var AXES=[["Eq-computation","wRPS dist"],["Strategic","kuhn"],["Time","intertemporal"],["Framing","cross-mod"],["Time-risk","allais"]];
  var KEYS=["eqc","kuhn","intertemporal","cross_modality","allais"];
  var VMAX=0.75, cx=245, cy=215, R=145, N=5;
  function mid(m){return m.replace(/[^a-z0-9]/gi,"_");}
  function P(i,v){var a=(-90+72*i)*Math.PI/180,r=R*Math.min(v,VMAX)/VMAX;return [cx+r*Math.cos(a),cy+r*Math.sin(a)];}
  function ring(f){var p=[];for(var i=0;i<N;i++){var a=(-90+72*i)*Math.PI/180,r=R*f;p.push((cx+r*Math.cos(a)).toFixed(1)+","+(cy+r*Math.sin(a)).toFixed(1));}return p.join(" ");}
  ...
  function svg(models){
    ...
    for(var i=0;i<N;i++){
      var tip=P(i,VMAX),a=(-90+72*i)*Math.PI/180;
    ...
    if(models.length===1){
      var m=models[0];
      for(var i=0;i<N;i++){var v=DATA[m].vals[KEYS[i]],xy=P(i,v),a=(-90+72*i)*Math.PI/180;
  ...
  function readout(models){
    ...
    h+='</table><p style="font-size:.78em;color:#999;margin:.4em 0">raw exploitability, lower = better &#183; radius scaled to 0.75 &#183; <sup>&dagger;</sup> lighter sampling (N=32,K=1,4 seeds) &middot; rps omitted (external-RNG => 0 for all; eq-computation = weighted-RPS distribution)</p>';
    return h;
  }
