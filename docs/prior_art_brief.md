...
- **OracleDecomposable as a generalizing abstraction**: AERead applies Eq. 4 mechanics across a **depth-selected family of use cases** — v0 first paper: C2 ProductProcurementGame (discrete-action procurement under qualitative evidence) + C8 SimplePricingGame (continuous-action pricing under demand uncertainty); roadmap: D2 bargaining (TERMS-Bench wrap) + D3 VendorSelectionGame (single-agent risk/uncertainty) + C1 persona-fit (13F IRL portfolio case) + agentic marketplace + NegotiationGame + PricingCompetitionGame when current results make those directions necessary. Demonstrating the same decomposition across two **action-space-distinct** decisions (discrete vs continuous) in v0 is the "generalization" claim — genuine structural variation, not just uncertainty-type variation. The candidate pool is a question map, not a case-count target. TERMS-Bench by itself does not stake this.
- **Cross-axis integration**: TERMS-Bench's intervention-decomposition is single-axis (information). AERead's 5-axis taxonomy (information × consistency × calibration × computational × meta-cognitive) integrates Δ_inf with 4 other axes via a pre-registered factorial intervention design; cross-decomposition produces a per-(model, task) attribution matrix with main-effect cells + pairwise interaction cells + residual. **Whether main effects explain a high fraction of variance is a per-domain empirical hypothesis** (working hypothesis: 60-75% for discrete-action procurement (C2 v0); 50-70% for continuous-action pricing under demand uncertainty (C8 v0); declining to 25-50% for long-horizon agents — full per-domain table in [methodology_open_questions.md § Q7](methodology_open_questions.md#q7--how-much-of-the-per-model-task-gap-do-layers-1--2-explain-whats-the-residual)). Pre-registered with falsification criteria per domain; not assumed.
...
TERMS-Bench focuses on bargaining. Their findings — that αcue < 0 universally — raise a question they don't pursue: **does this cue-fooling pattern hold across non-bargaining economic decisions, or is it bargaining-specific?** AERead's depth-selected Layer 3 design (v0: C2 ProductProcurementGame + C8 SimplePricingGame; roadmap directions only when a result creates the need) is the empirical test. If αcue < 0 holds across structurally distinct domains, that's a load-bearing methodology-paper finding ("LLM agents lose surplus when given cues — across procurement, pricing, vendor selection") and a citable generalization claim downstream researchers can build on.
...
- **Multi-environment Layer 3 structure**: AERead plans to **wrap EconEvals** as v0.5 Layer 3 D-dimensions only where those wraps answer a live diagnostic question (D4 procurement / D5 pricing / D6 scheduling per [`layer3_candidates.md`](layer3_candidates.md) A1/A2/A3) — using EconEvals's environments as Cat D candidates in AERead's depth-first Layer 3 framing.
