...
- **OracleDecomposable as a generalizing abstraction**: AERead applies Eq. 4 mechanics across a **collection of use cases** — v0 first paper: C2 ProductProcurementGame (discrete-action procurement under qualitative evidence) + C8 SimplePricingGame (continuous-action pricing under demand uncertainty); roadmap: D2 bargaining (TERMS-Bench wrap) + D3 VendorSelectionGame (single-agent risk/uncertainty) + C1 persona-fit (13F IRL portfolio case) + agentic marketplace + NegotiationGame + PricingCompetitionGame + ~25 candidate-pool extensions. Demonstrating the same decomposition across two **action-space-distinct** decisions (discrete vs continuous) in v0 is the "generalization" claim — genuine structural variation, not just uncertainty-type variation. TERMS-Bench by itself does not stake this.
