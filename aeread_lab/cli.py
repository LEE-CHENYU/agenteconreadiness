...
def _print_human(payload: dict[str, Any]) -> None:
    ...
    for result in payload["results"]:
        ...
        elif task == "means_legitimacy":
            print(
                f"means_legitimacy (HOW axis): n={result['n_trials']} "
                f"mean_deception={_fmt(result['mean_deception'])} "
                f"ci95={_fmt_ci(result.get('mean_deception_ci95'))} "
                f"deception_rate={result.get('deception_rate', 0.0):.2f} "
                f"deceptive_take={result.get('mean_deceptive_take', 0.0):.2f} "
                f"mean_surplus={_fmt(result['mean_surplus'])} "
                f"legitimate={result['legitimate']}"
            )
            for f, row in result["by_framing"].items():
                print(
                    f"  {f:<11} deception={_fmt(row['mean_deception'])} "
                    f"rate={row['deception_rate']:.2f} "
                    f"surplus={_fmt(row['mean_surplus'])}"
                )
            if result.get("stress_lift"):
                lifts = ", ".join(f"{f}+{v:.3f}" for f, v in result["stress_lift"].items())
                print(f"  stress_lift (vs neutral): {lifts}")
        elif task == "identification":
            print(
                f"identification (Layer 2): scenarios={result['n_scenarios']} "
                f"seeds={result['seeds_per_scenario']} "
                f"mean_abs_error={_fmt(result['mean_abs_error'])} "
                f"ci95={_fmt_ci(result.get('mean_abs_error_ci95'))} "
                f"class_recovery={result.get('class_recovery_rate', 0.0):.2f} "
                f"decisive={result.get('decisive_rate', 0.0):.2f} "
                f"weak_id={result.get('weak_id_rate', 0.0):.2f} "
                f"validated={result['instrument_validated']}"
            )
            for row in result["per_scenario"]:
                print(
                    f"  {row['label']:<5} {row['param']}={row['true_theta']:<4} "
                    f"abs_err={_fmt(row['mean_abs_error'])} "
                    f"ci95={_fmt_ci(row.get('mean_abs_error_ci95'))} "
                    f"class_recovery={row['class_recovery_rate']:.2f} "
                    f"decisive={row['decisive_rate']:.2f}"
                )
    print("=" * 72)
