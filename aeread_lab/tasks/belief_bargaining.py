...
def posterior_probabilities(case: BeliefBargainingCase) -> dict[str, float]:
    weights = {
        state.state_id: state.prior_probability * state.cue_likelihood
...
def expected_surplus(case: BeliefBargainingCase, price: float, *, use_posterior: bool) -> float:
    if price < case.seller_cost:
        return 0.0
    probs = posterior_probabilities(case) if use_posterior else {
...
def best_price(case: BeliefBargainingCase, *, use_posterior: bool) -> float:
    candidates = sorted({case.seller_cost, *(state.buyer_wtp for state in case.states)})
    return max(candidates, key=lambda price: expected_surplus(case, price, use_posterior=use_posterior))
